...
__export(typespecs_exports, {
  ...
  INVOKE_API_VERSION: () => INVOKE_API_VERSION,
...
var codeBundleSchema = import_zod7.z.object({
  ...
  location: import_zod7.z.object({
    ...
    position: import_zod7.z.union([import_zod7.z.literal("task"), import_zod7.z.object({ score: import_zod7.z.number() })])
  }),
...
// typespecs/api_types.ts
(0, import_zod_to_openapi4.extendZodWithOpenApi)(import_zod8.z);
var auditSourcesSchema = import_zod8.z.enum(VALID_SOURCES);
function generateBaseEventOpSchema(objectType) {
  ...
  return import_zod8.z.object({
    ...
    context: import_zod8.z.object({
      caller_functionname: import_zod8.z.string().nullish().describe(
        `The function in code which created the ${eventDescription} event`
      ),
      caller_filename: import_zod8.z.string().nullish().describe(
        `Name of the file in code where the ${eventDescription} event was created`
      ),
      caller_lineno: import_zod8.z.number().int().nullish().describe(
    ...
    span_id: import_zod8.z.string().uuid().describe(
    ...
    span_parents: import_zod8.z.string().array().nullish().describe(
    ...
    root_span_id: import_zod8.z.string().uuid().describe(
      `The \`span_id\` of the root of the trace this ${eventDescription} event belongs to`
    ),
    span_attributes: import_zod8.z.object({
      name: import_zod8.z.string().nullish().describe("Name of the span, for display purposes only"),
      type: import_zod8.z.enum(spanTypeAttributeValues).nullish().describe("Type of the span, for display purposes only")
    }).catchall(customTypes.any).nullish().describe(
      "Human-identifying attributes of the span, such as name, type, etc."
    ),
    [OBJECT_DELETE_FIELD]: import_zod8.z.boolean().nullish().describe(
...
function generateBaseEventFeedbackSchema(objectType) {
  ...
  return import_zod8.z.object({
    ...
    scores: import_zod8.z.record(import_zod8.z.number().min(0).max(1).nullish()).nullish().describe(
    ...
    tags: import_zod8.z.array(import_zod8.z.string()).nullish().describe("A list of tags to log"),
    comment: import_zod8.z.string().nullish().describe(
...
var fetchLimitSchema = import_zod8.z.number().int().nonnegative().describe(
  [
...
var maxXactIdSchema = import_zod8.z.string().describe(fetchPaginationCursorDescription);
var maxRootSpanIdSchema = import_zod8.z.string().describe(fetchPaginationCursorDescription);
var fetchPaginationCursorSchema = import_zod8.z.string().describe(
  [
...
var versionSchema = import_zod8.z.string().describe(
  [
...
var pathTypeFilterSchema = import_zod8.z.object({
  type: import_zod8.z.literal("path_lookup").describe("Denotes the type of filter as a path-lookup filter"),
  path: import_zod8.z.string().array().describe(
...
var sqlTypeFilterSchema = import_zod8.z.object({
  type: import_zod8.z.literal("sql_filter").describe("Denotes the type of filter as a sql-type filter"),
  expr: import_zod8.z.string().describe(
...
var allFetchFiltersSchema = import_zod8.z.union([pathTypeFilterSchema, sqlTypeFilterSchema]).array().describe(
...
var fetchEventsRequestSchema = import_zod8.z.object({
...
function makeFetchEventsResponseSchema(objectType, eventSchema) {
  ...
  return import_zod8.z.object({
    ...
    cursor: import_zod8.z.string().nullish().describe(
      [
        "Pagination cursor",
...
var experimentEventSchema = import_zod8.z.object({
  id: experimentEventBaseSchema.shape.id,
  dataset_record_id: import_zod8.z.string().nullish().describe(
...
var datasetEventSchema = import_zod8.z.object({
...
var promptSessionEventSchema = import_zod8.z.object({
...
var projectLogsEventSchema = import_zod8.z.object({
  ...
  log_id: import_zod8.z.literal("g").describe("A literal 'g' which identifies the log as a project log"),
...
var mergeEventSchema = import_zod8.z.object({
  [IS_MERGE_FIELD]: customTypes.literalTrue.describe(isMergeDescription),
  [MERGE_PATHS_FIELD]: import_zod8.z.string().array().array().nullish().describe(
    [
...
var replacementEventSchema = import_zod8.z.object({
  [IS_MERGE_FIELD]: customTypes.literalFalse.nullish().describe(isMergeDescription),
  [PARENT_ID_FIELD]: import_zod8.z.string().nullish().describe(
    [
...
function makeInsertEventSchemas(objectType, insertSchema) {
  ...
  const eventSchema = import_zod8.z.union([replaceVariantSchema, mergeVariantSchema]).describe(`${capitalize(article)} ${eventDescription} event`).openapi(`Insert${eventSchemaName}Event`);
...
var insertEventsResponseSchema = import_zod8.z.object({
  row_ids: import_zod8.z.string().array().describe(
...
var feedbackExperimentItemSchema = import_zod8.z.object({
...
var feedbackDatasetItemSchema = import_zod8.z.object({
...
var feedbackProjectLogsItemSchema = import_zod8.z.object({
...
var feedbackPromptItemSchema = import_zod8.z.object({
...
var feedbackFunctionItemSchema = import_zod8.z.object({
...
function makeCrossObjectIndividualRequestSchema(objectType) {
  ...
  const insertObject = import_zod8.z.object({
  ...
  return import_zod8.z.record(import_zod8.z.string().uuid(), insertObject).nullish().describe(
...
function makeCrossObjectIndividualResponseSchema(objectType) {
  return import_zod8.z.record(import_zod8.z.string().uuid(), insertEventsResponseSchema).nullish().describe(
    `A mapping from ${objectType} id to row ids for inserted \`events\``
  );
}
var crossObjectInsertRequestSchema = import_zod8.z.object({
...
var crossObjectInsertResponseSchema = import_zod8.z.object({
...
var summarizeScoresParamSchema = import_zod8.z.boolean().describe(
  "Whether to summarize the scores and metrics. If false (or omitted), only the metadata will be returned."
);
var comparisonExperimentIdParamSchema = import_zod8.z.string().uuid().describe(
...
var summarizeDataParamSchema = import_zod8.z.boolean().describe(
  "Whether to summarize the data. If false (or omitted), only the metadata will be returned."
);
var summarizeExperimentResponseSchema = import_zod8.z.object({
  project_name: import_zod8.z.string().describe("Name of the project that the experiment belongs to"),
  experiment_name: import_zod8.z.string().describe("Name of the experiment"),
  project_url: import_zod8.z.string().url().describe("URL to the project's page in the Braintrust app"),
  experiment_url: import_zod8.z.string().url().describe("URL to the experiment's page in the Braintrust app"),
  comparison_experiment_name: import_zod8.z.string().nullish().describe("The experiment which scores are baselined against"),
  scores: import_zod8.z.record(
    import_zod8.z.object({
      name: import_zod8.z.string().describe("Name of the score"),
      score: import_zod8.z.number().min(0).max(1).describe("Average score across all examples"),
      diff: import_zod8.z.number().min(-1).max(1).optional().describe(
        "Difference in score between the current and comparison experiment"
      ),
      improvements: import_zod8.z.number().int().min(0).describe("Number of improvements in the score"),
      regressions: import_zod8.z.number().int().min(0).describe("Number of regressions in the score")
    }).describe("Summary of a score's performance").openapi("ScoreSummary")
  ).nullish().describe("Summary of the experiment's scores"),
  metrics: import_zod8.z.record(
    import_zod8.z.object({
      name: import_zod8.z.string().describe("Name of the metric"),
      metric: import_zod8.z.number().describe("Average metric across all examples"),
      unit: import_zod8.z.string().describe("Unit label for the metric"),
      diff: import_zod8.z.number().optional().describe(
        "Difference in metric between the current and comparison experiment"
      ),
      improvements: import_zod8.z.number().int().min(0).describe("Number of improvements in the metric"),
      regressions: import_zod8.z.number().int().min(0).describe("Number of regressions in the metric")
...
var summarizeDatasetResponseSchema = import_zod8.z.object({
  project_name: import_zod8.z.string().describe("Name of the project that the dataset belongs to"),
  dataset_name: import_zod8.z.string().describe("Name of the dataset"),
  project_url: import_zod8.z.string().url().describe("URL to the project's page in the Braintrust app"),
  dataset_url: import_zod8.z.string().url().describe("URL to the dataset's page in the Braintrust app"),
  data_summary: import_zod8.z.object({
    total_records: import_zod8.z.number().int().min(0).describe("Total number of records in the dataset")
...
var useFunctionSchema = import_zod9.z.object({
  api_version: import_zod9.z.number().optional().default(INVOKE_API_VERSION)
}).and(functionIdSchema);
var invokeFunctionSchema = useFunctionSchema.and(
  import_zod9.z.object({
    input: import_zod9.z.any().optional(),
    parent: import_zod9.z.string().optional(),
