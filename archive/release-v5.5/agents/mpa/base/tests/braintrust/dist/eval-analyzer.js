...
async function fetchLatestExperiment() {
    ...
    if (!apiKey)
        throw new Error("BRAINTRUST_API_KEY not found");
    const headers = {
        Authorization: `Bearer ${apiKey}`,
        "Content-Type": "application/json",
    };
    // Get project ID
    const projectResp = await fetch(`${API_BASE}/project?project_name=${encodeURIComponent(PROJECT_NAME)}`, { headers });
    ...
    const expResp = await fetch(`${API_BASE}/experiment?project_id=${projectId}&limit=1`, { headers });
...
function analyzeScorePatterns(results) {
    const scorerStats = {};
...
async function generatePromptSuggestions(patterns) {
...
function generateReport(results, patterns, suggestions) {
    const totalTests = results.length;
    const avgOverallScore = results.reduce((sum, r) => {
...
async function main() {
    ...
    for (const pattern of patterns) {
        const status = pattern.avgScore >= 0.8 ? "✓" : pattern.avgScore >= 0.6 ? "⚠" : "✗";
        console.log(`   ${status} ${pattern.scorer}: ${(pattern.avgScore * 100).toFixed(1)}% avg (${pattern.lowScoreCount} low scores)`);
    }
...
//# sourceMappingURL=eval-analyzer.js.map