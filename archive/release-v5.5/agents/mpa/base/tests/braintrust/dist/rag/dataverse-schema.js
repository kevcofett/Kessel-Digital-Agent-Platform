 ...
 */
export function migrateBenchmarkToV6(v55, defaults = {}) {
    const now = new Date();
 ...
 */
function inferSeasonality(date) {
 ...
 */
export function calculateFreshnessScore(kpiCode, periodEnd) {
    const now = new Date();
 ...
 */
export function buildBenchmarkFilter(vertical, channel, kpi, minFreshness) {
    const filters = [];
 ...
 */
export function buildBenchmarkSelect(includeHistorical = false) {
    const fields = [
        'mpa_vertical_code',
        'mpa_channel_code',
        'mpa_kpi_code',
 ...
 */
export function formatBenchmarkResponse(benchmark) {
    const lines = [];
    // Basic range
 ...
 */
function formatMetricValue(value, kpiCode) {
 ...
 */
export function buildGeographyFilter(country, geoType, minPopulation, maxRank) {
    const filters = [];
 ...
 */
export function buildIABFilter(tier, parentCode, vertical) {
    const filters = [];
    if (tier) {
 ...
 */
export function buildPlatformTaxonomyFilter(platform, taxonomyType, vertical) {
    const filters = [];
 ...
 */
export function buildBehavioralFilter(category, intentLevel, platform) {
    const filters = [];
    if (category) {
 ...
 */
export function buildContextualFilter(category, brandSafetyTier, iabCode) {
    const filters = [];
    if (category) {
...
//# sourceMappingURL=dataverse-schema.js.map