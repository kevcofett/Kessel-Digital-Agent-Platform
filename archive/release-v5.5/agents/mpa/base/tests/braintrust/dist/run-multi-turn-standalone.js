...
function generateReport(result) {
    ...
    lines.push(`Steps Completed: ${result.finalStepState.completedSteps.length}/10 (${result.finalStepState.completedSteps.join(", ") || "none"})`);
    lines.push(`Duration: ${formatDuration(result.executionMetadata.totalDurationMs)}`);
    lines.push(`Tokens Used: ${result.executionMetadata.totalTokensUsed.toLocaleString()}`);
    // Failures
...
async function main() {
    ...
    console.log("-".repeat(60));
    console.log("Scenario".padEnd(30) +
        "Score".padEnd(10) +
        "Threshold".padEnd(12) +
        "Status");
    console.log("-".repeat(60));
    ...
    for (const result of results) {
        ...
        console.log(result.scenario.name.slice(0, 29).padEnd(30) +
...
//# sourceMappingURL=run-multi-turn-standalone.js.map