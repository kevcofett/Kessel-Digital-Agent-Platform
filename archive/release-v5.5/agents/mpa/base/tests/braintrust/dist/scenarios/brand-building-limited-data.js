...
export const brandBuildingLimitedDataScenario = {
    ...
    expectedEvents: [
        {
            type: "idk_protocol",
            description: "Agent models with assumptions when user says 'I don't know'",
        },
        {
    ...
    failureConditions: [
        {
            ...
            description: "Agent obsesses over CAC/ROAS when objective is brand awareness",
            type: "custom_pattern",
            severity: "major",
            scorePenalty: 0.15,
        ...
        },
        {
            ...
            description: "Agent loops on question when user says 'I don't know' instead of modeling",
            type: "loop_detection",
            severity: "major",
            scorePenalty: 0.15,
            terminateOnDetect: false,
        ...
        },
        {
            ...
            description: "Agent recommends performance metrics (ROAS, CAC) over brand metrics (reach, frequency, lift)",
            type: "custom_pattern",
            severity: "warning",
            scorePenalty: 0.1,
            terminateOnDetect: false,
        },
        {
            ...
            description: "Agent asks 5+ questions about unit economics for awareness campaign",
        ...
        customCriteria: [
            {
                ...
                description: "Agent discussed brand metrics (reach, frequency, awareness, lift) appropriately",
                evaluator: (result) => {
                    ...
                    const brandMetricsFound = brandMetricPatterns.filter((p) => p.test(allResponses)).length;
            ...
            },
            {
                ...
                description: "Agent did not obsess over CAC when objective is brand awareness",
                evaluator: (result) => {
                    // Count CAC-focused questions in steps 1-3
                    let cacQuestions = 0;
                    const earlyStepTurns = result.turns.filter((t) => t.currentStep <= 3);
            ...
            },
            {
                ...
                description: "Agent offered to model with assumptions when user lacked data",
                evaluator: (result) => {
                    const allResponses = result.turns
                        .map((t) => t.agentResponse)
                        .join(" ");
            ...
            },
            {
                ...
                description: "Agent explained how awareness campaigns differ from performance",
                evaluator: (result) => {
                    const allResponses = result.turns
                        .map((t) => t.agentResponse)
                        .join(" ");
            ...
            },
            {
                ...
                description: "Agent recommended appropriate measurement approach for awareness",
                evaluator: (result) => {
                    const allResponses = result.turns
                        .map((t) => t.agentResponse)
                        .join(" ");
                    ...
                    const measurementMentioned = measurementPatterns.filter((p) => p.test(allResponses)).length;
            ...
            },
            {
                ...
                description: "Agent used plain language appropriate for intermediate user",
                evaluator: (result) => {
                    ...
                    for (const turn of result.turns) {
                        for (const pattern of jargonTerms) {
                            if (pattern.test(turn.agentResponse)) {
                                ...
                                const hasExplanation = /(?:which is|that is|meaning|stands for|also known as)/i.test(turn.agentResponse);
                                if (!hasExplanation) {
