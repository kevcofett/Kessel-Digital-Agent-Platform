...
export const highStakesPerformancePersona = {
    ...
    knownData: {
        ...
        audienceDescription: "Small business owners processing $10K-100K monthly transactions",
        hasGeography: true,
    ...
    failureConditions: [
        {
            ...
            description: "Agent accepts $40 CAC target without questioning feasibility",
            type: "custom_pattern",
            severity: "critical",
            scorePenalty: 0.25,
        ...
        },
        {
            ...
            description: "Agent makes efficiency claims without citing industry benchmarks",
            type: "custom_pattern",
            severity: "major",
            scorePenalty: 0.1,
            terminateOnDetect: false,
        ...
        customCriteria: [
            {
                ...
                evaluator: (result) => {
                    ...
                    const hasCalculation = /\$?2[,\s]?(?:000[,\s]?000|M|m|million)?\s*[÷\/]\s*50[,\s]?(?:000|K|k)?\s*=\s*\$?40/i.test(allResponses) ||
                        /(?:equals?|=|implies?|projects? to|that['']?s|works out to)\s*\$?40\s*(?:per|CAC|acquisition|cost|merchant)/i.test(allResponses);
            ...
            },
            {
                ...
                description: "Agent connected Step 2 economics to later budget/channel decisions",
                evaluator: (result) => {
                    // Check if later steps reference earlier efficiency insights
                    const laterStepTurns = result.turns.filter((t) => t.currentStep >= 4 && t.agentResponse.length > 100);
                    if (laterStepTurns.length === 0)
                        return true; // Not applicable if didn't reach later steps
