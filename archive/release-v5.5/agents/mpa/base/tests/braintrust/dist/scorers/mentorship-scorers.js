...
export function scoreProactiveCalculation(agentResponse, availableData) {
    ...
    const askedWhenCouldCalculate = asksForDerivedData.some((p) => p.test(agentResponse));
...
export function scoreBenchmarkCitation(agentResponse, stepContext) {
    ...
    const hasStrongCitation = strongCitationPatterns.some((p) => p.test(agentResponse));
    const hasWeakCitation = weakCitationPatterns.some((p) => p.test(agentResponse));
...
export async function scoreCriticalThinking(userInput, agentResponse, context) {
    ...
    const prompt = CRITICAL_THINKING_JUDGE_PROMPT.replace("{user_input}", userInput)
...
export async function scoreStrategicSynthesis(agentResponse, conversationContext) {
    ...
    const prompt = STRATEGIC_SYNTHESIS_JUDGE_PROMPT.replace("{conversation_context}", contextSummary)
