 ...
 */
function extractQuantitativeClaims(response) {
    const claims = [];
 ...
 */
function hasExplicitSource(response, claim) {
 ...
 */
function hasImplicitSource(response, claim) {
    const claimIndex = response.indexOf(claim);
    if (claimIndex === -1) {
        return false;
    }
 ...
 */
export function scoreSourceCitation(response) {
    ...
    if (explicitCount > 0)
        rationale += `${explicitCount} with explicit source, `;
    if (implicitCount > 0)
        rationale += `${implicitCount} with implicit source, `;
    if (unattributedCount > 0)
        rationale += `${unattributedCount} unattributed`;
...
//# sourceMappingURL=source-citation.js.map