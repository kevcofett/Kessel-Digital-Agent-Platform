 ...
 */
export function isVerticalSupported(vertical) {
 ...
 */
export function scoreBenchmarkVerticalCoverage(output, userVertical) {
    ...
    const channelPatterns = SUPPORTED_CHANNELS.map(c => new RegExp(c.replace('_', '[ _]?'), 'i'));
 ...
 */
export function scoreWebSearchTrigger(output, input) {
 ...
 */
export function scoreKbRoutingValidation(output, input, retrievedDocs) {
    ...
    const normalizeDoc = (d) => d.toLowerCase().replace(/[_\s]/g, '').replace(/v6_0|v60|\.txt/g, '');
    const retrievedNormalized = retrievedDocs.map(normalizeDoc);
    const expectedNormalized = expectedDocs.map(normalizeDoc);
    const matchedDocs = expectedNormalized.filter(e => retrievedNormalized.some(r => r.includes(e) || e.includes(r)));
 ...
 */
export function scoreConfidenceLevelAttribution(output) {
...
//# sourceMappingURL=v6-scorers.js.map