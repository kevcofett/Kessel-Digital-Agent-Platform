...
__export(typespecs_exports, {
  ...
  promptEventSchema: () => promptEventSchema,
...
var messageSchema = import_zod3.z.object({
  ...
  function_call: import_zod3.z.union([import_zod3.z.string(), functionCallSchema]).optional(),
  tool_calls: import_zod3.z.array(toolCallSchema).optional()
});
var promptBlockDataSchema = import_zod3.z.union([
  import_zod3.z.object({
    type: import_zod3.z.literal("completion"),
    content: import_zod3.z.string()
  }),
  import_zod3.z.object({
    type: import_zod3.z.literal("chat"),
    messages: import_zod3.z.array(messageSchema),
    tools: import_zod3.z.string().optional()
  })
]);
var braintrustModelParamsSchema = import_zod3.z.object({
  use_cache: import_zod3.z.boolean().optional()
});
var openAIModelParamsSchema = import_zod3.z.object({
  temperature: import_zod3.z.number(),
  top_p: import_zod3.z.number().optional(),
  max_tokens: import_zod3.z.number().optional(),
  frequency_penalty: import_zod3.z.number().optional(),
  presence_penalty: import_zod3.z.number().optional(),
  response_format: import_zod3.z.union([import_zod3.z.literal(null), import_zod3.z.object({ type: import_zod3.z.literal("json_object") })]).optional(),
  tool_choice: import_zod3.z.union([
    import_zod3.z.literal("auto"),
    import_zod3.z.literal("none"),
    import_zod3.z.object({
      type: import_zod3.z.literal("function"),
      function: import_zod3.z.object({ name: import_zod3.z.string() })
    })
  ]).optional()
});
var anthropicModelParamsSchema = import_zod3.z.object({
  max_tokens: import_zod3.z.number(),
  temperature: import_zod3.z.number(),
  top_p: import_zod3.z.number().optional(),
  top_k: import_zod3.z.number().optional(),
  max_tokens_to_sample: import_zod3.z.number().optional().describe("This is a legacy parameter that should not be used.")
});
var googleModelParamsSchema = import_zod3.z.object({
  temperature: import_zod3.z.number(),
  maxOutputTokens: import_zod3.z.number().optional(),
  topP: import_zod3.z.number().optional(),
  topK: import_zod3.z.number().optional()
});
var jsCompletionParamsSchema = import_zod3.z.object({});
...
function generateBaseTableSchema(objectName, opts) {
  ...
  return import_zod4.z.object({
    id: import_zod4.z.string().uuid().describe(`Unique identifier for the ${objectName}`),
    project_id: import_zod4.z.string().uuid().describe(
      `Unique identifier for the project that the ${objectName} belongs under`
    ),
    name: import_zod4.z.string().describe(nameDescription),
    description: import_zod4.z.string().nullish().describe(`Textual description of the ${objectName}`),
    ...
    user_id: import_zod4.z.string().uuid().nullish().describe(`Identifies the user who created the ${objectName}`),
    metadata: import_zod4.z.record(customTypes.any).nullish().describe(`User-controlled metadata about the ${objectName}`)
  });
}
var userBaseSchema = generateBaseTableSchema("user");
var userSchema = import_zod4.z.object({
  id: userBaseSchema.shape.id,
  given_name: import_zod4.z.string().nullish().describe("Given name of the user"),
  family_name: import_zod4.z.string().nullish().describe("Family name of the user"),
  email: import_zod4.z.string().nullish().describe("The user's email"),
  avatar_url: import_zod4.z.string().nullish().describe("URL of the user's Avatar image"),
  created: userBaseSchema.shape.created
}).strict().openapi("User");
var organizationBaseSchema = generateBaseTableSchema("organization");
var organizationSchema = import_zod4.z.object({
  id: organizationBaseSchema.shape.id,
  name: organizationBaseSchema.shape.name.nullish(),
  api_url: import_zod4.z.string().nullish(),
  created: organizationBaseSchema.shape.created
}).strict().openapi("Organization");
var memberSchema = import_zod4.z.object({
  org_id: organizationSchema.shape.id,
  user_id: userSchema.shape.id
}).strict().openapi("Member");
var meSchema = import_zod4.z.object({
  id: userSchema.shape.id,
  organizations: import_zod4.z.object({
...
var apiKeySchema = import_zod4.z.object({
  ...
  key_hash: import_zod4.z.string(),
  name: apiKeyBaseSchema.shape.name,
  preview_name: import_zod4.z.string(),
...
var projectSchema = import_zod4.z.object({
  ...
  org_id: import_zod4.z.string().uuid().describe(
...
var promptSchema = import_zod4.z.object({
  ...
  slug: import_zod4.z.string().describe("Unique identifier for the prompt"),
  ...
  tags: import_zod4.z.array(import_zod4.z.string()).nullish().describe("A list of tags for the prompt")
});
var repoInfoSchema = import_zod4.z.object({
  commit: import_zod4.z.string().nullish().describe("SHA of most recent commit"),
  branch: import_zod4.z.string().nullish().describe("Name of the branch the most recent commit belongs to"),
  tag: import_zod4.z.string().nullish().describe("Name of the tag on the most recent commit"),
  dirty: import_zod4.z.boolean().nullish().describe(
    "Whether or not the repo had uncommitted changes when snapshotted"
  ),
  author_name: import_zod4.z.string().nullish().describe("Name of the author of the most recent commit"),
  author_email: import_zod4.z.string().nullish().describe("Email of the author of the most recent commit"),
  commit_message: import_zod4.z.string().nullish().describe("Most recent commit message"),
  commit_time: import_zod4.z.string().nullish().describe("Time of the most recent commit"),
  git_diff: import_zod4.z.string().nullish().describe(
...
var experimentSchema = import_zod4.z.object({
  ...
  commit: import_zod4.z.string().nullish().describe("Commit, taken directly from `repo_info.commit`"),
  base_exp_id: import_zod4.z.string().uuid().nullish().describe(
    "Id of default base experiment to compare against when viewing this experiment"
  ),
  deleted_at: experimentBaseSchema.shape.deleted_at,
  dataset_id: import_zod4.z.string().uuid().nullish().describe(
    "Identifier of the linked dataset, or null if the experiment is not linked to a dataset"
  ),
  dataset_version: import_zod4.z.string().nullish().describe(
    "Version number of the linked dataset the experiment was run against. This can be used to reproduce the experiment after the dataset has been modified."
  ),
  public: import_zod4.z.boolean().describe(
...
var privilegeEnum = import_zod4.z.enum([
  "create",
  "read",
  "update",
  "delete",
...
var roleSchema = import_zod4.z.object({
  id: roleBaseSchema.shape.id,
  org_id: import_zod4.z.string().uuid().nullish().describe(
    [
  ...
  member_privileges: import_zod4.z.array(privilegeEnum).nullish().describe("Privileges which belong to this role"),
  member_roles: import_zod4.z.array(import_zod4.z.string().uuid()).nullish().describe(
    [
...
var teamSchema = import_zod4.z.object({
  ...
  member_users: import_zod4.z.array(import_zod4.z.string().uuid()).nullish().describe("Ids of users which belong to this team"),
  member_teams: import_zod4.z.array(import_zod4.z.string().uuid()).nullish().describe(
    [
...
var aclObjectTypeEnum = import_zod4.z.enum([
  "organization",
  "project",
  "experiment",
  "dataset",
...
var aclObjectSchema = import_zod4.z.object({
  id: aclBaseSchema.shape.id,
  object_type: aclObjectTypeEnum,
  object_id: import_zod4.z.string().uuid().describe("The id of the object the ACL applies to"),
  _object_org_id: import_zod4.z.string().uuid().describe("The organization the ACL's referred object belongs to"),
  created: aclBaseSchema.shape.created
}).strict();
var aclUserObjectSchema = import_zod4.z.object({
  user_id: import_zod4.z.string().uuid().describe("Id of the user the ACL applies to")
}).strict();
var aclTeamObjectSchema = import_zod4.z.object({
  team_id: import_zod4.z.string().uuid().describe("Id of the team the ACL applies to")
}).strict();
var aclPrivilegeObjectSchema = import_zod4.z.object({
  privilege: privilegeEnum.describe("Privilege the ACL grants")
}).strict();
var aclRoleObjectSchema = import_zod4.z.object({
  role_id: import_zod4.z.string().uuid().describe("Role the ACL grants")
}).strict();
var aclSchema = import_zod4.z.union([
...
var appLimitSchema = import_zod4.z.number().int().nonnegative().describe("Limit the number of objects to return");
function generateBaseTableOpSchema(objectName) {
  return import_zod4.z.object({
    org_name: import_zod4.z.string().nullish().describe(
...
var startingAfterSchema = import_zod4.z.string().uuid().describe(
  [
    "Pagination cursor id.",
    "For example, if the final item in the last page you fetched had an id of `foo`, pass `starting_after=foo` to fetch the next page. Note: you may only pass one of `starting_after` and `ending_before`"
  ].join("\n\n")
).openapi("StartingAfter");
var endingBeforeSchema = import_zod4.z.string().uuid().describe(
  [
    "Pagination cursor id.",
...
var createDatasetSchema = import_zod4.z.object({
...
var patchDatasetSchema = import_zod4.z.object({
...
var patchPromptSchema = import_zod4.z.object({
...
var createTeamSchema = import_zod4.z.object({
...
var createAclSchema = import_zod4.z.union([
...
var auditSourcesSchema = import_zod5.z.enum(VALID_SOURCES);
function generateBaseEventOpSchema(objectType) {
  ...
  return import_zod5.z.object({
    ...
    [TRANSACTION_ID_FIELD]: import_zod5.z.string().describe(
    ...
    tags: import_zod5.z.array(import_zod5.z.string()).nullish().describe("A list of tags to log"),
    scores: import_zod5.z.record(import_zod5.z.number().min(0).max(1).nullish()).nullish(),
    metadata: import_zod5.z.record(customTypes.any).nullish().describe(
      "A dictionary with additional data about the test example, model outputs, or just about anything else that's relevant, that you can use to help find and analyze examples later. For example, you could log the `prompt`, example's `id`, or anything else that would be useful to slice/dice later. The values in `metadata` can be any JSON-serializable type, but its keys must be strings"
    ),
    metrics: import_zod5.z.object({
      start: import_zod5.z.number().nullish().describe(
        `A unix timestamp recording when the section of code which produced the ${eventDescription} event started`
      ),
      end: import_zod5.z.number().nullish().describe(
    ...
    context: import_zod5.z.object({
      caller_functionname: import_zod5.z.string().nullish().describe(
        `The function in code which created the ${eventDescription} event`
      ),
      caller_filename: import_zod5.z.string().nullish().describe(
        `Name of the file in code where the ${eventDescription} event was created`
      ),
      caller_lineno: import_zod5.z.number().int().nullish().describe(
    ...
    span_id: import_zod5.z.string().describe(
    ...
    span_parents: import_zod5.z.string().array().nullish().describe(
    ...
    root_span_id: import_zod5.z.string().describe(
      `The \`span_id\` of the root of the trace this ${eventDescription} event belongs to`
    ),
    span_attributes: import_zod5.z.object({
      name: import_zod5.z.string().nullish().describe("Name of the span, for display purposes only"),
      type: import_zod5.z.nativeEnum(SpanTypeAttribute).nullish().describe("Type of the span, for display purposes only")
    }).catchall(customTypes.any).nullish().describe(
      "Human-identifying attributes of the span, such as name, type, etc."
    ),
    [OBJECT_DELETE_FIELD]: import_zod5.z.boolean().nullish().describe(
...
function generateBaseEventFeedbackSchema(objectType) {
  ...
  return import_zod5.z.object({
    ...
    scores: import_zod5.z.record(import_zod5.z.number().min(0).max(1).nullish()).nullish().describe(
    ...
    tags: import_zod5.z.array(import_zod5.z.string()).nullish().describe("A list of tags to log"),
    comment: import_zod5.z.string().nullish().describe(
...
var fetchLimitSchema = import_zod5.z.number().int().nonnegative().describe(
  [
...
var maxXactIdSchema = import_zod5.z.string().describe(fetchPaginationCursorDescription);
var maxRootSpanIdSchema = import_zod5.z.string().describe(fetchPaginationCursorDescription);
var versionSchema = import_zod5.z.string().describe(
  [
...
var pathTypeFilterSchema = import_zod5.z.object({
  type: import_zod5.z.literal("path_lookup").describe("Denotes the type of filter as a path-lookup filter"),
  path: import_zod5.z.string().array().describe(
...
var sqlTypeFilterSchema = import_zod5.z.object({
  type: import_zod5.z.literal("sql_filter").describe("Denotes the type of filter as a sql-type filter"),
  expr: import_zod5.z.string().describe(
...
var allFetchFiltersSchema = import_zod5.z.union([pathTypeFilterSchema, sqlTypeFilterSchema]).array().describe(
...
var fetchEventsRequestSchema = import_zod5.z.object({
...
var experimentEventSchema = import_zod5.z.object({
  id: experimentEventBaseSchema.shape.id,
  dataset_record_id: import_zod5.z.string().nullish().describe(
...
var datasetEventSchema = import_zod5.z.object({
...
var projectLogsEventSchema = import_zod5.z.object({
  ...
  log_id: import_zod5.z.literal("g").describe("A literal 'g' which identifies the log as a project log"),
...
var promptEventSchema = import_zod5.z.object({
  ...
  org_id: projectSchema.shape.org_id,
  project_id: projectSchema.shape.id,
  log_id: import_zod5.z.literal("p").describe("A literal 'p' which identifies the log as a prompt entry"),
  name: import_zod5.z.string().describe("The name of the prompt"),
  slug: import_zod5.z.string().describe("The slug of the prompt"),
  description: import_zod5.z.string().describe("The description of the prompt"),
...
var mergeEventSchema = import_zod5.z.object({
  [IS_MERGE_FIELD]: customTypes.literalTrue.describe(isMergeDescription),
  [MERGE_PATHS_FIELD]: import_zod5.z.string().array().array().nullish().describe(
    [
...
var replacementEventSchema = import_zod5.z.object({
  [IS_MERGE_FIELD]: customTypes.literalFalse.nullish().describe(isMergeDescription),
  [PARENT_ID_FIELD]: import_zod5.z.string().nullish().describe(
    [
...
function makeInsertEventSchemas(objectType, insertSchema) {
  ...
  const eventSchema = import_zod5.z.union([replaceVariantSchema, mergeVariantSchema]).describe(`${capitalize(article)} ${eventDescription} event`).openapi(`Insert${eventSchemaName}Event`);
...
var insertEventsResponseSchema = import_zod5.z.object({
  row_ids: import_zod5.z.string().array().describe(
...
var feedbackExperimentItemSchema = import_zod5.z.object({
...
var feedbackDatasetItemSchema = import_zod5.z.object({
...
var feedbackProjectLogsItemSchema = import_zod5.z.object({
...
var feedbackPromptItemSchema = import_zod5.z.object({
...
function makeCrossObjectIndividualRequestSchema(objectType) {
  ...
  const insertObject = import_zod5.z.object({
  ...
  return import_zod5.z.record(import_zod5.z.string().uuid(), insertObject).nullish().describe(
...
function makeCrossObjectIndividualResponseSchema(objectType) {
  return import_zod5.z.record(import_zod5.z.string().uuid(), insertEventsResponseSchema).nullish().describe(
    `A mapping from ${objectType} id to row ids for inserted \`events\``
  );
}
var crossObjectInsertRequestSchema = import_zod5.z.object({
...
var crossObjectInsertResponseSchema = import_zod5.z.object({
...
var summarizeScoresParamSchema = import_zod5.z.boolean().describe(
  "Whether to summarize the scores and metrics. If false (or omitted), only the metadata will be returned."
);
var comparisonExperimentIdParamSchema = import_zod5.z.string().uuid().describe(
...
var summarizeDataParamSchema = import_zod5.z.boolean().describe(
  "Whether to summarize the data. If false (or omitted), only the metadata will be returned."
);
var summarizeExperimentResponseSchema = import_zod5.z.object({
  project_name: import_zod5.z.string().describe("Name of the project that the experiment belongs to"),
  experiment_name: import_zod5.z.string().describe("Name of the experiment"),
  project_url: import_zod5.z.string().url().describe("URL to the project's page in the Braintrust app"),
  experiment_url: import_zod5.z.string().url().describe("URL to the experiment's page in the Braintrust app"),
  comparison_experiment_name: import_zod5.z.string().nullish().describe("The experiment which scores are baselined against"),
  scores: import_zod5.z.record(
    import_zod5.z.object({
      name: import_zod5.z.string().describe("Name of the score"),
      score: import_zod5.z.number().min(0).max(1).describe("Average score across all examples"),
      diff: import_zod5.z.number().min(-1).max(1).describe(
        "Difference in score between the current and comparison experiment"
      ),
      improvements: import_zod5.z.number().int().min(0).describe("Number of improvements in the score"),
      regressions: import_zod5.z.number().int().min(0).describe("Number of regressions in the score")
    }).describe("Summary of a score's performance").openapi("ScoreSummary")
  ).nullish().describe("Summary of the experiment's scores"),
  metrics: import_zod5.z.record(
    import_zod5.z.object({
      name: import_zod5.z.string().describe("Name of the metric"),
      metric: import_zod5.z.number().describe("Average metric across all examples"),
      unit: import_zod5.z.string().describe("Unit label for the metric"),
      diff: import_zod5.z.number().describe(
        "Difference in metric between the current and comparison experiment"
      ),
      improvements: import_zod5.z.number().int().min(0).describe("Number of improvements in the metric"),
      regressions: import_zod5.z.number().int().min(0).describe("Number of regressions in the metric")
...
var summarizeDatasetResponseSchema = import_zod5.z.object({
  project_name: import_zod5.z.string().describe("Name of the project that the dataset belongs to"),
  dataset_name: import_zod5.z.string().describe("Name of the dataset"),
  project_url: import_zod5.z.string().url().describe("URL to the project's page in the Braintrust app"),
  dataset_url: import_zod5.z.string().url().describe("URL to the dataset's page in the Braintrust app"),
  data_summary: import_zod5.z.object({
    total_records: import_zod5.z.number().int().min(0).describe("Total number of records in the dataset")
