...
var imageURLSchema = z3.object({
  url: z3.string(),
  detail: z3.union([z3.literal("auto"), z3.literal("low"), z3.literal("high")]).optional()
});
...
var chatCompletionContentSchema = z3.union([
  z3.string().default(""),
  z3.array(chatCompletionContentPartSchema)
]);
...
var codeBundleSchema = z7.object({
  ...
  location: z7.object({
    ...
    eval_name: z7.string(),
    position: z7.union([z7.literal("task"), z7.object({ score: z7.number() })])
  }),
  bundle_id: z7.string()
});
...
// typespecs/api_types.ts
extendZodWithOpenApi4(z8);
var auditSourcesSchema = z8.enum(VALID_SOURCES);
function generateBaseEventOpSchema(objectType) {
  ...
  return z8.object({
    ...
    [TRANSACTION_ID_FIELD]: z8.string().describe(
    ...
    context: z8.object({
      caller_functionname: z8.string().nullish().describe(
        `The function in code which created the ${eventDescription} event`
      ),
      caller_filename: z8.string().nullish().describe(
        `Name of the file in code where the ${eventDescription} event was created`
      ),
      caller_lineno: z8.number().int().nullish().describe(
    ...
    span_id: z8.string().uuid().describe(
    ...
    span_parents: z8.string().array().nullish().describe(
    ...
    root_span_id: z8.string().uuid().describe(
      `The \`span_id\` of the root of the trace this ${eventDescription} event belongs to`
    ),
    span_attributes: z8.object({
      name: z8.string().nullish().describe("Name of the span, for display purposes only"),
      type: z8.enum(spanTypeAttributeValues).nullish().describe("Type of the span, for display purposes only")
    }).catchall(customTypes.any).nullish().describe(
      "Human-identifying attributes of the span, such as name, type, etc."
    ),
    [OBJECT_DELETE_FIELD]: z8.boolean().nullish().describe(
...
function generateBaseEventFeedbackSchema(objectType) {
  ...
  return z8.object({
    ...
    scores: z8.record(z8.number().min(0).max(1).nullish()).nullish().describe(
    ...
    tags: z8.array(z8.string()).nullish().describe("A list of tags to log"),
    comment: z8.string().nullish().describe(
...
var fetchLimitSchema = z8.number().int().nonnegative().describe(
  [
...
var maxXactIdSchema = z8.string().describe(fetchPaginationCursorDescription);
var maxRootSpanIdSchema = z8.string().describe(fetchPaginationCursorDescription);
var fetchPaginationCursorSchema = z8.string().describe(
  [
...
var versionSchema = z8.string().describe(
  [
...
var pathTypeFilterSchema = z8.object({
  type: z8.literal("path_lookup").describe("Denotes the type of filter as a path-lookup filter"),
  path: z8.string().array().describe(
...
var sqlTypeFilterSchema = z8.object({
  type: z8.literal("sql_filter").describe("Denotes the type of filter as a sql-type filter"),
  expr: z8.string().describe(
...
var allFetchFiltersSchema = z8.union([pathTypeFilterSchema, sqlTypeFilterSchema]).array().describe(
...
var fetchEventsRequestSchema = z8.object({
...
function makeFetchEventsResponseSchema(objectType, eventSchema) {
  ...
  return z8.object({
    ...
    cursor: z8.string().nullish().describe(
      [
        "Pagination cursor",
...
var experimentEventSchema = z8.object({
  id: experimentEventBaseSchema.shape.id,
  dataset_record_id: z8.string().nullish().describe(
...
var datasetEventSchema = z8.object({
...
var promptSessionEventSchema = z8.object({
...
var projectLogsEventSchema = z8.object({
  ...
  log_id: z8.literal("g").describe("A literal 'g' which identifies the log as a project log"),
...
var mergeEventSchema = z8.object({
  [IS_MERGE_FIELD]: customTypes.literalTrue.describe(isMergeDescription),
  [MERGE_PATHS_FIELD]: z8.string().array().array().nullish().describe(
    [
...
var replacementEventSchema = z8.object({
  [IS_MERGE_FIELD]: customTypes.literalFalse.nullish().describe(isMergeDescription),
  [PARENT_ID_FIELD]: z8.string().nullish().describe(
    [
...
function makeInsertEventSchemas(objectType, insertSchema) {
  ...
  const eventSchema = z8.union([replaceVariantSchema, mergeVariantSchema]).describe(`${capitalize(article)} ${eventDescription} event`).openapi(`Insert${eventSchemaName}Event`);
  const requestSchema = z8.object({
...
var insertEventsResponseSchema = z8.object({
  row_ids: z8.string().array().describe(
...
var feedbackExperimentItemSchema = z8.object({
...
var feedbackDatasetItemSchema = z8.object({
...
var feedbackProjectLogsItemSchema = z8.object({
...
var feedbackPromptItemSchema = z8.object({
...
var feedbackFunctionItemSchema = z8.object({
...
function makeCrossObjectIndividualRequestSchema(objectType) {
  ...
  return z8.record(z8.string().uuid(), insertObject).nullish().describe(
...
function makeCrossObjectIndividualResponseSchema(objectType) {
  return z8.record(z8.string().uuid(), insertEventsResponseSchema).nullish().describe(
    `A mapping from ${objectType} id to row ids for inserted \`events\``
  );
}
var crossObjectInsertRequestSchema = z8.object({
...
var crossObjectInsertResponseSchema = z8.object({
...
var summarizeScoresParamSchema = z8.boolean().describe(
  "Whether to summarize the scores and metrics. If false (or omitted), only the metadata will be returned."
);
var comparisonExperimentIdParamSchema = z8.string().uuid().describe(
...
var summarizeDataParamSchema = z8.boolean().describe(
  "Whether to summarize the data. If false (or omitted), only the metadata will be returned."
);
var summarizeExperimentResponseSchema = z8.object({
  project_name: z8.string().describe("Name of the project that the experiment belongs to"),
  experiment_name: z8.string().describe("Name of the experiment"),
  project_url: z8.string().url().describe("URL to the project's page in the Braintrust app"),
  experiment_url: z8.string().url().describe("URL to the experiment's page in the Braintrust app"),
  comparison_experiment_name: z8.string().nullish().describe("The experiment which scores are baselined against"),
  scores: z8.record(
    z8.object({
      name: z8.string().describe("Name of the score"),
      score: z8.number().min(0).max(1).describe("Average score across all examples"),
      diff: z8.number().min(-1).max(1).optional().describe(
        "Difference in score between the current and comparison experiment"
      ),
      improvements: z8.number().int().min(0).describe("Number of improvements in the score"),
      regressions: z8.number().int().min(0).describe("Number of regressions in the score")
    }).describe("Summary of a score's performance").openapi("ScoreSummary")
  ).nullish().describe("Summary of the experiment's scores"),
  metrics: z8.record(
    z8.object({
      name: z8.string().describe("Name of the metric"),
      metric: z8.number().describe("Average metric across all examples"),
      unit: z8.string().describe("Unit label for the metric"),
      diff: z8.number().optional().describe(
        "Difference in metric between the current and comparison experiment"
      ),
      improvements: z8.number().int().min(0).describe("Number of improvements in the metric"),
      regressions: z8.number().int().min(0).describe("Number of regressions in the metric")
...
var summarizeDatasetResponseSchema = z8.object({
  project_name: z8.string().describe("Name of the project that the dataset belongs to"),
  dataset_name: z8.string().describe("Name of the dataset"),
  project_url: z8.string().url().describe("URL to the project's page in the Braintrust app"),
  dataset_url: z8.string().url().describe("URL to the dataset's page in the Braintrust app"),
  data_summary: z8.object({
    total_records: z8.number().int().min(0).describe("Total number of records in the dataset")
...
var useFunctionSchema = z9.object({
  api_version: z9.number().optional().default(INVOKE_API_VERSION)
}).and(functionIdSchema);
var invokeFunctionSchema = useFunctionSchema.and(
  z9.object({
    input: z9.any().optional(),
