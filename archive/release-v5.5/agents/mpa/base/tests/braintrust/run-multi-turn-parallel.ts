...
interface ParsedArgs {
  scenario?: string;
  category: "quick" | "full" | "all" | "phase1" | "targeting" | "multi-audience" | "reforecasting" | "adis" | "advanced-analytics";
  verbose: boolean;
  model?: string;
...
function parseArgs(): ParsedArgs {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case "--category":
      case "-c":
        result.category = args[++i] as ParsedArgs["category"];
        break;
      case "--verbose":
      case "-v":
        result.verbose = true;
...
function printProgress(state: ProgressState) {
...
async function main() {
  ...
  await runWithConcurrency(scenarios, args.concurrency, async (scenario) => {
    ...
    try {
      ...
      console.log(
        `${status} ${scenario.name} | ${(result.compositeScore * 100).toFixed(1)}% | ${result.totalTurns} turns | ${formatDuration(result.executionMetadata.totalDurationMs)}`
      );
  ...
  for (const result of results) {
    ...
    console.log(
      result.scenario.name.slice(0, 34).padEnd(35) +
