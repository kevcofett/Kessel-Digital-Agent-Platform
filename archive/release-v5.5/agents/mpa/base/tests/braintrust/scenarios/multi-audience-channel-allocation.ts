 ...
 */
export const multiAudienceChannelPersona: UserPersona = {
  ...
  knownData: {
    hasObjective: true,
    objective:
      "Drive enrollments across three distinct learner segments with optimized channel mix per segment",
    hasVolume: true,
 ...
 */
export const multiAudienceChannelAllocationScenario: TestScenario = {
  id: "multi-audience-channel-allocation",
  name: "Multi-Audience with Segment-Specific Channels",
  category: "phase1-quality",
  description:
    "Advanced test: Multiple target audiences with different channel allocations " +
  ...
  openingMessage:
    "I'm the VP of Marketing at EduPath Online Learning. We need a sophisticated media " +
  ...
  failureConditions: [
    {
      ...
      description:
        "Agent recommends same channel mix for distinct segments",
      type: "custom_pattern",
      severity: "critical",
      scorePenalty: 0.25,
    ...
    },
    {
      ...
      description:
        "Agent recommends TikTok for corporate L&D or LinkedIn for college-age",
      type: "custom_pattern",
      severity: "major",
      scorePenalty: 0.15,
      terminateOnDetect: false,
    },
    {
      ...
      description:
        "Agent recommends channels without explaining fit to segment behaviors",
      type: "custom_pattern",
      severity: "major",
      scorePenalty: 0.1,
      terminateOnDetect: false,
    },
    {
      ...
      description:
        "Agent doesn't break down budget by segment AND by channel",
      type: "custom_pattern",
      severity: "warning",
      scorePenalty: 0.1,
      terminateOnDetect: false,
    ...
    customCriteria: [
      {
        ...
        evaluator: (result) => {
          ...
          const careerChannels = /(?:career|upskill|28.42).*(?:linkedin|podcast|search)/i.test(
            allResponses
          );
          const collegeChannels = /(?:college|18.24|gen.z).*(?:tiktok|instagram|influencer)/i.test(
            allResponses
          );
          const corporateChannels = /(?:corporate|l&d|hr).*(?:linkedin|abm|trade)/i.test(
            allResponses
          );
      ...
      },
      {
        ...
        evaluator: (result) => {
          ...
          const wrongFound = wrongPatterns.filter((p) =>
            p.test(allResponses)
          ).length;
          return wrongFound === 0;
        },
