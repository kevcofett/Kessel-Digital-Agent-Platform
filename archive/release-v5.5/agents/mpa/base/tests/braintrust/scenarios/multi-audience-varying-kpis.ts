 ...
 */
export const multiAudienceVaryingKpisPersona: UserPersona = {
  ...
  knownData: {
    hasObjective: true,
    objective:
      "Drive different objectives for three customer segments: awareness for conquest, leads for in-market, loyalty for existing",
    hasVolume: true,
    ...
    audienceDescription:
      "Three segments with different objectives: " +
 ...
 */
export const multiAudienceVaryingKpisScenario: TestScenario = {
  id: "multi-audience-varying-kpis",
  name: "Multi-Audience with Different KPIs Per Segment",
  category: "phase1-quality",
  description:
    "Most complex multi-audience test: Three segments with completely different " +
  ...
  openingMessage:
    "I'm the CMO at Velocity Auto Group - we operate 12 dealerships across 3 brands. " +
  ...
  failureConditions: [
    {
      ...
      description:
        "Agent applies a single KPI (like CAC) across all segments",
      type: "custom_pattern",
      severity: "critical",
      scorePenalty: 0.25,
    ...
    },
    {
      ...
      description:
        "Agent applies CAC/CPL metrics to the conquest/awareness segment",
      type: "custom_pattern",
      severity: "major",
      scorePenalty: 0.15,
      terminateOnDetect: false,
    },
    {
      ...
      description:
        "Agent applies reach/frequency metrics as primary KPI for in-market segment",
      type: "custom_pattern",
      severity: "major",
      scorePenalty: 0.15,
      terminateOnDetect: false,
    },
    {
      ...
      description:
        "Agent treats loyalty segment economics same as acquisition",
      type: "custom_pattern",
      severity: "warning",
      scorePenalty: 0.1,
      terminateOnDetect: false,
    ...
    customCriteria: [
      {
        ...
        evaluator: (result) => {
          ...
          const conquestKpi = /(?:conquest|awareness).*(?:reach|frequency|brand lift|impression)/i.test(
            allResponses
          );
          const inmarketKpi = /(?:in.market|lead).*(?:cpl|lead|form|test drive|vdp)/i.test(
            allResponses
          );
          const loyaltyKpi = /(?:loyalty|retention|existing).*(?:service|trade.in|retention|ro|repair order)/i.test(
            allResponses
          );
...
export const multiAudienceVaryingKpisContext = {
  budget: 4500000,
  funnel: "consideration" as const, // Mixed funnel
