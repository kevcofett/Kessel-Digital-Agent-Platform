 ...
 */
const CITATION_PATTERNS: Record<CitationType, RegExp[]> = {
 ...
 */
export function scoreCitationPresence(agentResponse: string): {
  score: number;
  metadata: Record<string, unknown>;
} {
  // Find all quantitative claims
  const quantitativeClaims = QUANTITATIVE_CLAIM_PATTERNS.filter((p) =>
...
export function scoreCitationAccuracy(
  ...
  const detectedCitations: CitationType[] = [];
  const potentialMisattributions: string[] = [];

  // Check KB citations
  const citesKB = CITATION_PATTERNS[CitationType.KB_BENCHMARK].some((p) =>
    p.test(agentResponse)
  );
  if (citesKB) {
    detectedCitations.push(CitationType.KB_BENCHMARK);
    if (context.kbDataRetrieved === false) {
      potentialMisattributions.push(
        "Cited KB data but no KB retrieval occurred"
      );
    }
  }

  // Check web search citations
  const citesWebSearch = CITATION_PATTERNS[CitationType.WEB_SEARCH].some((p) =>
    p.test(agentResponse)
  );
  if (citesWebSearch) {
    ...
    if (context.webSearchPerformed === false) {
      potentialMisattributions.push(
        "Cited web search but no search was performed"
      );
    }
  }

  // Check user input citations
  const citesUserInput = CITATION_PATTERNS[CitationType.USER_INPUT].some((p) =>
    p.test(agentResponse)
  );
  if (citesUserInput) {
  ...
  // Check API citations
  const citesApi = CITATION_PATTERNS[CitationType.DIRECT_API].some((p) =>
    p.test(agentResponse)
  );
  if (citesApi) {
    ...
    if (context.apiDataAvailable === false) {
      potentialMisattributions.push(
        "Cited API data but no API was available"
      );
    }
  }
 ...
 */
export function scoreCitationConsistency(agentResponse: string): {
  score: number;
  metadata: Record<string, unknown>;
} {
 ...
 */
export function scoreConfidenceLevel(agentResponse: string): {
  score: number;
  metadata: Record<string, unknown>;
} {
  ...
  const hasEstimate = CITATION_PATTERNS[CitationType.AGENT_ESTIMATE].some((p) =>
    p.test(agentResponse)
  );
 ...
 */
export async function scoreCitationQuality(
  userMessage: string,
  agentResponse: string,
  context: {
    kbDataRetrieved?: boolean;
