 ...
 */
const FALLBACK_EXPECTATIONS: Record<
  FailureType,
  {
    acknowledgmentPatterns: RegExp[];
    fallbackPatterns: RegExp[];
    recommendationPatterns: RegExp[];
  }
> = {
 ...
 */
export function scoreFailureAcknowledgment(
  agentResponse: string,
  failureType: FailureType
): { score: number; metadata: Record<string, unknown> } {
  const expectations = FALLBACK_EXPECTATIONS[failureType];
  const acknowledged = expectations.acknowledgmentPatterns.some((p) =>
 ...
 */
export function scoreFallbackBehavior(
  agentResponse: string,
  failureType: FailureType
): { score: number; metadata: Record<string, unknown> } {
  const expectations = FALLBACK_EXPECTATIONS[failureType];
  const usedFallback = expectations.fallbackPatterns.some((p) =>
    p.test(agentResponse)
  );
 ...
 */
export function scoreFollowUpRecommendation(
  agentResponse: string,
  failureType: FailureType
): { score: number; metadata: Record<string, unknown> } {
  const expectations = FALLBACK_EXPECTATIONS[failureType];
  const hasRecommendation = expectations.recommendationPatterns.some((p) =>
    p.test(agentResponse)
  );
 ...
 */
export function scoreForwardProgress(agentResponse: string): {
  score: number;
 ...
 */
export async function scoreGracefulDegradation(
  userMessage: string,
  agentResponse: string,
  failureContext: {
    failureType: FailureType;
