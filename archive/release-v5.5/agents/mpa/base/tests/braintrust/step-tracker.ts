...
export class StepTracker {
  private stepDefinitions: Map<number, StepDefinition>;
  ...
  initializeState(): StepTrackingState {
    const collectedData: Record<number, StepCollectedData> = {};
  ...
  detectCurrentStep(
    userMessage: string,
    agentMessage: string,
    currentState: StepTrackingState
  ): number {
    const combinedText = `${userMessage} ${agentMessage}`.toLowerCase();

    // First check for explicit step transition signals in agent message
    const stepTransitionMatch = agentMessage.match(
      /(?:step|moving to|let'?s discuss|now for)\s*(\d+)|(?:step)\s*(\d+)/i
    );
    if (stepTransitionMatch) {
      ...
      if (mentionedStep >= 1 && mentionedStep <= 10) {
        ...
        if (
          mentionedStep === currentState.currentStep + 1 ||
    ...
    if (this.detectStepCompletion(agentMessage, currentState.currentStep)) {
      ...
      if (nextStep <= 10) {
        ...
        if (nextStepDef) {
          const matchCount = nextStepDef.detectionPatterns.filter((p) =>
            p.test(combinedText)
          ).length;
          if (matchCount > 0) {
            return nextStep;
  ...
  updateState(
    ...
    const newState: StepTrackingState = {
      ...currentState,
    ...
    const requiredMet = stepData.requiredDataPoints.filter((dp) =>
      collectedKeys.some((ck) => ck.toLowerCase().includes(dp.toLowerCase()))
    );
  ...
  private extractAgentData(
    agentMessage: string,
    stepData: StepCollectedData
  ): void {
    // Budget extraction
    const budgetMatch = agentMessage.match(
      /\$?([\d,]+)\s*(?:k|K|thousand)?(?:\s*budget)?/
    );
    if (budgetMatch) {
      let value = parseFloat(budgetMatch[1].replace(/,/g, ""));
      if (/k|K|thousand/i.test(budgetMatch[0])) value *= 1000;
      if (value > 10000) {
    ...
    const cacMatch = agentMessage.match(
      /\$([\d,]+)\s*(?:per customer|cac|acquisition cost|cost per)/i
    );
    if (cacMatch) {
      stepData.collectedDataPoints.impliedCac = parseFloat(
        cacMatch[1].replace(/,/g, "")
      );
    ...
    // Volume extraction
    const volumeMatch = agentMessage.match(
      /([\d,]+)\s*(customers?|leads?|users?)/i
    );
    if (volumeMatch) {
      stepData.collectedDataPoints.volumeTarget = parseInt(
    ...
    // Objective detection
    if (
      /customer acquisition|acquire.*customers|grow.*customer base/i.test(
        agentMessage
      )
    ) {
    ...
    // KPI detection
    if (
      /new customers?|customer count|customers? acquired/i.test(agentMessage)
    ) {
  ...
  private detectStepCompletion(
    ...
    const stepCompletionPatterns: Record<number, RegExp[]> = {
      1: [
