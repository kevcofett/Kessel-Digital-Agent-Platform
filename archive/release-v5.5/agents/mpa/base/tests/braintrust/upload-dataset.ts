...
async function uploadDataset() {
  ...
  let projects: { objects?: Array<{ id: string; name: string }> };
  try {
  ...
  if (projects.objects && projects.objects.length > 0) {
    ...
    const newProject = JSON.parse(createProjectText) as { id: string };
  ...
  let datasetResponse = await fetch(
    `${API_BASE}/dataset?project_id=${projectId}&dataset_name=${encodeURIComponent(datasetName)}`,
    { headers }
  );
  let datasets = await datasetResponse.json() as { objects?: Array<{ id: string; name: string }> };
  ...
  if (datasets.objects && datasets.objects.length > 0) {
    ...
    const newDataset = await createDatasetResp.json() as { id: string };
  ...
  const verifyResult = await verifyResponse.json() as { events?: unknown[] };
