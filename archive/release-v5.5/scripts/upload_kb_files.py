...
Uploads knowledge base files to SharePoint document library using Microsoft Graph API.

Usage:
    python upload_kb_files.py [--library LIBRARY] [--overwrite] [-v]

Examples:
    python upload_kb_files.py                      # Upload to default MediaPlanningKB
    python upload_kb_files.py --library CustomKB   # Upload to specific library
    python upload_kb_files.py --no-overwrite       # Skip existing files
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Upload KB files to SharePoint"
    )
    parser.add_argument(
        "--library",
        type=str,
        help="Target SharePoint library name (default: from environment.json)"
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
    ...
    print("=" * 60)
    print("MPA KB Files SharePoint Upload")
    print("=" * 60)
    ...
    if success_count == len(results):
        print("\n[SUCCESS] All KB files uploaded successfully")
    else:
