...
export class BaseKBImpactTracker {
     ...
     */
    async recordRetrieval(sessionId, query, retrievedDocs) {
        const recordIds = [];
     ...
     */
    async markAsUsed(sessionId, usedDocumentIds) {
     ...
     */
    async recordQuality(sessionId, qualityScore) {
        const records = this.sessionRecords.get(sessionId) || [];
        for (const record of records) {
     ...
     */
    async recalculateDocumentImpact(documentId) {
        ...
        const notUsedRecords = baselineRecords.filter(r => !r.chunkIds.some(c => records.some(rec => rec.chunkIds.includes(c))) &&
    ...
    async getDocumentsNeedingAttention() {
        const impacts = await this.getAllDocumentImpacts();
        return impacts.filter(i => i.recommendedAction !== 'keep' || i.confidence < this.config.confidenceThreshold);
    }
    // ==========================================================================
    // HELPER METHODS
    // ==========================================================================
    calculateConfidence(usedCount, baselineCount) {
    ...
    determineAction(impactScore, confidence, totalUsages) {
        // Not enough data
    ...
    analyzeTopics(records) {
        const topicScores = new Map();
        for (const record of records) {
            if (record.responseQuality === undefined)
                continue;
...
//# sourceMappingURL=base-kb-impact-tracker.js.map