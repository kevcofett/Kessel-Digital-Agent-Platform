...
export class SelfCritique {
    ...
    config;
    critiqueCount = 0;
    constructor(llmProvider, criteria, config) {
     ...
     */
    async critique(userQuery, agentResponse, metadata) {
...
Output your evaluation in the following JSON format:
{
  ...
  "suggestions": ["<suggestion1>", "<suggestion2>"]
     ...
     */
    buildCritiqueRequest(userQuery, agentResponse) {
...
Provide your evaluation in JSON format.`;
    ...
    fallbackParseCritique(text) {
        ...
        if (textLower.includes('excellent') || textLower.includes('outstanding'))
            score = 0.95;
        else if (textLower.includes('good') || textLower.includes('well done'))
            score = 0.85;
        else if (textLower.includes('adequate') || textLower.includes('acceptable'))
            score = 0.70;
        else if (textLower.includes('poor') || textLower.includes('lacking'))
            score = 0.50;
        else if (textLower.includes('unacceptable') || textLower.includes('incorrect'))
            score = 0.30;
        return {
            overallScore: score,
    ...
    normalizeIssueType(type) {
        const validTypes = ['accuracy', 'completeness', 'clarity', 'citation', 'calculation', 'other'];
    ...
    normalizeSeverity(severity) {
        ...
        if (normalized === 'minor' || normalized === 'low')
            return 'minor';
        if (normalized === 'major' || normalized === 'high' || normalized === 'critical')
            return 'major';
        return 'moderate';
    }
    /**
     ...
     */
    async critiqueBatch(interactions) {
        const results = [];
        for (const interaction of interactions) {
            const critique = await this.critique(interaction.userQuery, interaction.agentResponse, interaction.metadata);
