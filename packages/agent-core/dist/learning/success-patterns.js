...
export class SuccessPatterns {
    ...
    indexCache = null;
    constructor(storage, storageConfig, learningConfig) {
     ...
     */
    async extractFromCritique(critique) {
     ...
     */
    detectPatternType(query, response) {
     ...
     * Extract response elements that made it effective
     */
    extractResponseElements(critique) {
        const elements = [];
     ...
     */
    extractTriggerConditions(query) {
        const conditions = [];
        const queryLower = query.toLowerCase();
     ...
     */
    generatePatternDescription(type, elements) {
     ...
     */
    async findSimilarPattern(type, triggerConditions) {
        ...
        for (const candidate of candidates) {
            ...
            if (pattern) {
                ...
                const overlap = pattern.triggerConditions.filter(tc => triggerConditions.some(newTc => newTc.includes(tc.replace('Query contains ', '').replace('"', ''))));
     ...
     */
    async reinforcePattern(pattern, critique) {
        // Update pattern
        pattern.observationCount++;
        pattern.lastReinforcedAt = new Date();
        // Update average score
        pattern.averageScore = ((pattern.averageScore * (pattern.observationCount - 1)) + critique.overallScore) / pattern.observationCount;
        ...
        if (pattern.examples.length < 5) {
            ...
            const lowestIndex = pattern.examples.reduce((minIdx, ex, idx, arr) => ex.score < arr[minIdx].score ? idx : minIdx, 0);
     ...
     */
    async getRelevantPatterns(query, limit = 3) {
        await this.loadIndex();
        if (!this.indexCache)
            return [];
        const queryLower = query.toLowerCase();
    ...
    async loadPattern(id) {
        ...
        try {
            ...
            const pattern = await this.storage.readJSON(path);
    ...
    async loadIndex() {
        if (this.indexCache)
            return;
        try {
            this.indexCache = await this.storage.readJSON(this.storageConfig.indexPath);
        }
        catch {
...
//# sourceMappingURL=success-patterns.js.map