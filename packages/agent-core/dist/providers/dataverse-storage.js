...
export class DataverseStorageProvider {
    ...
    constructor(config) {
        ...
        if ('environmentUrl' in config) {
            ...
            if (!config.dataverseClientId)
                missing.push('DATAVERSE_CLIENT_ID');
            if (!config.dataverseClientSecret)
    ...
    async readDocument(path) {
        ...
        return data.value[0][this.columnMap.content];
    }
    /**
    ...
    async listDocuments(directory) {
        ...
        const data = await response.json();
        return data.value.map(record => record[this.columnMap.path]);
    }
    /**
    ...
    async findDocumentByPath(path) {
        ...
        try {
            ...
            return { id: data.value[0][this.columnMap.id] };
        }
        catch {
            return null;
        }
...
// FACTORY FUNCTION
// ============================================================================
export function createDataverseStorageProvider(config) {
