...
export class LLMJudgeScorer extends BaseScorer {
  ...
  private criteria: LLMJudgeCriteria[];
...
Output your evaluation in this JSON format:
{
   ...
   */
  private buildEvaluationRequest(transcript: ConversationTranscript): string {
...
Provide your evaluation in JSON format.`;
   ...
   */
  private parseEvaluation(text: string): ScorerResult {
    ...
    if (jsonMatch) {
      try {
        ...
        return this.createResult(
          (parsed.overallScore || 50) / 100,
          parsed.explanation || 'LLM evaluation completed',
          {
            breakdown,
   ...
   */
  private fallbackParse(text: string): ScorerResult {
    const textLower = text.toLowerCase();

    let score = 0.6;  // Default
    if (textLower.includes('excellent') || textLower.includes('outstanding')) score = 0.9;
    else if (textLower.includes('good')) score = 0.75;
    else if (textLower.includes('poor') || textLower.includes('lacking')) score = 0.4;

    return this.createResult(
      score,
      'Evaluated via fallback parsing',
      { issues: ['Could not parse structured LLM response'] }
    );
  }
   ...
   */
  private extractText(content: { type: string; text?: string }[]): string {
