...
export class KBEnhancementSystem {
     ...
     */
    async analyzeCritiques(critiques) {
     ...
     */
    trackGap(gap, newGaps) {
        // Check if similar gap exists in new gaps
        const existing = newGaps.find(g => g.type === gap.type &&
    ...
    mergeGaps(newGaps) {
        ...
        for (const newGap of newGaps) {
            const existing = this.indexCache.trackedGaps.find(g => g.type === newGap.type &&
    ...
    async generateProposals() {
        ...
        for (const gap of qualifiedGaps) {
            ...
            const existingEnhancement = this.indexCache.enhancements.find(e => e.type === gap.type &&
     ...
     */
    calculatePriority(gap, impact) {
        let priority = 3; // Default medium
        // Adjust based on occurrences
        if (gap.occurrences >= 10)
            priority--;
        if (gap.occurrences >= 5)
            priority--;
        // Adjust based on criticality
        if (impact.criticalityLevel === 'high')
            priority--;
        if (impact.criticalityLevel === 'low')
            priority++;
        // Clamp to 1-5
    ...
    async loadIndex() {
        if (this.indexCache)
            return;
        try {
            this.indexCache = await this.storage.readJSON(this.config.indexPath);
        }
        catch {
            this.indexCache = {
                enhancements: [],
     ...
     */
    async getPendingEnhancements() {
        await this.loadIndex();
        if (!this.indexCache)
            return [];
        const pending = [];
        const pendingStatuses = ['identified', 'validated', 'in_progress', 'pending_review'];
        for (const entry of this.indexCache.enhancements) {
            if (pendingStatuses.includes(entry.status)) {
                try {
                    const enhancement = await this.storage.readJSON(`${this.config.enhancementsPath}/${entry.id}.json`);
    ...
    async updateStatus(id, status) {
        ...
        if (entry) {
            entry.status = status;
            try {
                const enhancement = await this.storage.readJSON(`${this.config.enhancementsPath}/${id}.json`);
                enhancement.status = status;
                await this.storage.writeJSON(`${this.config.enhancementsPath}/${id}.json`, enhancement);
            }
            catch {
...
//# sourceMappingURL=kb-enhancement.js.map