...
export class ToolShim {
     ...
     */
    static createToolInstructions(tools) {
        if (tools.length === 0)
            return '';
...
`;
     ...
     */
    static parseActionRequests(response) {
     ...
     */
    static extractTextContent(response) {
     ...
     */
    static generateExampleInput(tool) {
        const example = {};
     ...
     */
    static formatParameters(schema) {
        const lines = [];
