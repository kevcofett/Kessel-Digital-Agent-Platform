...
import { KBEnhancementPipeline, EnhancementConfig, EvalResult, KBSuggestion } from '../learning/kb-enhancement';
...
async function testKBEnhancement(): Promise<void> {
  ...
  suggestions.forEach((s: KBSuggestion, i: number) => {
  ...
  const byFile = new Map<string, KBSuggestion[]>();
  ...
  prioritized.slice(0, 3).forEach((s: KBSuggestion, i: number) => {
