...
import { SuccessPatterns, SuccessPatternConfig, StoredPattern } from '../learning/success-patterns';
...
async function testSuccessPatterns(): Promise<void> {
  ...
  const config: SuccessPatternConfig = {
    enabled: true,
  ...
  const testPatterns: StoredPattern[] = [
    {
      id: 'test-1',
  ...
  const lowScorePattern: StoredPattern = {
    id: 'test-low',
    scenario: 'benchmark_query',
