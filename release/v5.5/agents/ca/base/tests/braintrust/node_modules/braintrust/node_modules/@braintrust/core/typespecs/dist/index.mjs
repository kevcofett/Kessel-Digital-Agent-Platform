...
var validRuntimesEnum = z9.enum(["node", "python"]);
var functionTypeEnum = z9.enum(["llm", "scorer", "task", "tool"]);
var runtimeContextSchema = z9.object({
  runtime: validRuntimesEnum,
  version: z9.string()
});
var functionIdSchema = z9.union([
  z9.object({
    function_id: z9.string().describe("The ID of the function"),
    version: z9.string().optional().describe("The version of the function")
  }).describe("Function id"),
  z9.object({
    project_name: z9.string().describe("The name of the project containing the function"),
    slug: z9.string().describe("The slug of the function"),
    version: z9.string().optional().describe("The version of the function")
  }).describe("Project name and slug"),
  z9.object({
    global_function: z9.string().describe(
  ...
  z9.object({
    prompt_session_id: z9.string().describe("The ID of the prompt session"),
    prompt_session_function_id: z9.string().describe("The ID of the function in the prompt session"),
    version: z9.string().optional().describe("The version of the function")
  }).describe("Prompt session id"),
  z9.object({
    inline_context: runtimeContextSchema,
    code: z9.string().describe("The inline code to execute"),
    name: z9.string().nullish().describe("The name of the inline code function")
  }).describe("Inline code function"),
  z9.object({
    inline_prompt: promptDataSchema,
    name: z9.string().nullish().describe("The name of the inline prompt")
  }).describe("Inline prompt definition")
]).describe("Options for identifying a function");
var useFunctionSchema = functionIdSchema;
var streamingModeEnum = z9.enum(["auto", "parallel"]);
var invokeFunctionNonIdArgsSchema = z9.object({
  input: customTypes.unknown.optional().describe(
    "Argument to the function, which can be any JSON serializable value"
  ),
  messages: z9.array(chatCompletionMessageParamSchema).optional().describe(
    "If the function is an LLM, additional messages to pass along to it"
  ),
  parent: z9.union([
    z9.object({
      object_type: z9.enum(["project_logs", "experiment"]),
      object_id: z9.string().describe("The id of the container object you are logging to"),
      row_ids: z9.object({
        id: z9.string().describe("The id of the row"),
        span_id: z9.string().describe("The span_id of the row"),
        root_span_id: z9.string().describe("The root_span_id of the row")
      }).nullish().describe("Identifiers for the row to to log a subspan under"),
      propagated_event: z9.record(customTypes.unknown).nullish().describe(
        "Include these properties in every span created under this parent"
      )
    }).describe("Span parent properties").openapi({ title: "span_parent_struct" }),
    z9.string().optional().describe(
      "The parent's span identifier, created by calling `.export()` on a span"
    )
  ]).describe("Options for tracing the function call"),
  stream: z9.boolean().nullish().describe(
...
var runEvalSchema = z9.object({
  project_id: z9.string().describe("Unique identifier for the project to run the eval in"),
  data: z9.union([
    z9.object({
      dataset_id: z9.string()
    }).describe("Dataset id"),
    z9.object({
      project_name: z9.string(),
      dataset_name: z9.string()
    }).describe("Project and dataset name")
  ]).describe("The dataset to use"),
  task: functionIdSchema.describe("The function to evaluate"),
  scores: z9.array(functionIdSchema).describe("The functions to score the eval on"),
  experiment_name: z9.string().optional().describe(
...
var baseSSEEventSchema = z9.object({
  id: z9.string().optional(),
  data: z9.string()
});
var sseTextEventSchema = baseSSEEventSchema.merge(
  z9.object({
    event: z9.literal("text_delta")
  })
);
var sseDataEventSchema = baseSSEEventSchema.merge(
  z9.object({
    event: z9.literal("json_delta")
  })
);
var sseErrorEventSchema = baseSSEEventSchema.merge(
  z9.object({
    event: z9.literal("error")
  })
);
var sseProgressEventSchema = baseSSEEventSchema.merge(
  z9.object({
    event: z9.literal("progress")
  })
);
var sseStartEventSchema = baseSSEEventSchema.omit({ data: true }).merge(
  z9.object({
    event: z9.literal("start"),
    data: z9.literal("")
  })
);
var sseDoneEventSchema = baseSSEEventSchema.omit({ data: true }).merge(
  z9.object({
    event: z9.literal("done"),
    data: z9.literal("")
  })
);
var functionObjectTypeEnum = z9.enum([
  "prompt",
  "tool",
  "scorer",
  "task"
]);
var functionFormatEnum = z9.enum(["llm", "code", "global"]);
var functionOutputTypeEnum = z9.enum(["completion", "score", "any"]);
var sseProgressEventDataSchema = z9.object({
  id: z9.string().describe("The id of the span this event is for"),
  object_type: functionObjectTypeEnum,
  format: functionFormatEnum,
  output_type: functionOutputTypeEnum,
  name: z9.string(),
  event: z9.enum(["text_delta", "json_delta", "error", "start", "done"]),
  data: z9.string()
  // This is the text_delta or json_delta
});
var callEventSchema = z9.union([
...
var scoreSchema = z9.union([
  z9.object({
    name: z9.string(),
    score: z9.number().min(0).max(1).nullable().default(null),
    // Sometimes we get an empty value over the wire
    metadata: z9.record(customTypes.unknown).optional().transform((data) => data != null ? data : void 0)
  }),
  z9.number().min(0).max(1),
  z9.boolean().transform((b) => b ? 1 : 0),
  z9.null()
]);
var ifExistsEnum = z9.enum(["error", "ignore", "replace"]);
var DEFAULT_IF_EXISTS = "error";

// typespecs/app_types.ts
extendZodWithOpenApi3(z10);
function generateBaseTableSchema(objectName, opts) {
  ...
  return z10.object({
    id: z10.string().uuid().describe(`Unique identifier for the ${objectName}`),
    project_id: z10.string().uuid().describe(
      `Unique identifier for the project that the ${objectName} belongs under`
    ),
    name: z10.string().describe(nameDescription),
    description: z10.string().nullish().describe(`Textual description of the ${objectName}`),
    ...
    user_id: z10.string().uuid().nullish().describe(`Identifies the user who created the ${objectName}`),
    metadata: z10.record(customTypes.unknown).nullish().describe(`User-controlled metadata about the ${objectName}`)
  });
}
var aclObjectTypeEnum = z10.enum([
  "organization",
  "project",
  "experiment",
  "dataset",
...
var userSchema = z10.object({
  ...
  given_name: z10.string().nullish().describe("Given name of the user"),
  family_name: z10.string().nullish().describe("Family name of the user"),
  email: z10.string().nullish().describe("The user's email"),
  avatar_url: z10.string().nullish().describe("URL of the user's Avatar image"),
  created: userBaseSchema.shape.created
}).openapi("User");
var organizationBaseSchema = generateBaseTableSchema("organization");
var organizationSchema = z10.object({
  id: organizationBaseSchema.shape.id,
  name: organizationBaseSchema.shape.name,
  api_url: z10.string().nullish(),
  is_universal_api: z10.boolean().nullish(),
  proxy_url: z10.string().nullish(),
  realtime_url: z10.string().nullish(),
  created: organizationBaseSchema.shape.created
}).openapi("Organization");
var maxOverWindowSchema = z10.object({
  window_size_days: z10.number().int().positive(),
  max_value: z10.number().nonnegative()
}).openapi("MaxOverWindow");
var resourcesSchema = z10.object({
  org_id: organizationSchema.shape.id,
  forbid_toggle_experiment_public_to_private: z10.boolean().nullish(),
  num_private_experiment_row_actions: maxOverWindowSchema.nullish(),
  forbid_insert_datasets: z10.boolean().nullish(),
  forbid_insert_prompt_sessions: z10.boolean().nullish(),
  forbid_access_sql_explorer: z10.boolean().nullish(),
  num_production_log_row_actions: maxOverWindowSchema.nullish(),
  num_dataset_row_actions: maxOverWindowSchema.nullish()
}).openapi("Resources");
var memberSchema = z10.object({
  org_id: organizationSchema.shape.id,
  user_id: userSchema.shape.id
}).openapi("Member");
var aiSecretBaseSchema = generateBaseTableSchema("AI secret");
var aiSecretSchema = z10.object({
  id: aiSecretBaseSchema.shape.id,
  created: aiSecretBaseSchema.shape.created,
  org_id: organizationSchema.shape.id,
  name: aiSecretBaseSchema.shape.name,
  type: z10.string().nullish(),
  metadata: z10.record(customTypes.unknown).nullish(),
  preview_secret: z10.string().nullish()
}).openapi("AISecret");
var envVarObjectTypeEnum = z10.enum(["organization", "project", "function"]).describe("The type of the object the environment variable is scoped for");
var envVarBaseSchema = generateBaseTableSchema("environment variable");
var envVarSchema = z10.object({
  id: envVarBaseSchema.shape.id,
  object_type: envVarObjectTypeEnum,
  object_id: z10.string().uuid().describe("The id of the object the environment variable is scoped for"),
  name: z10.string().describe("The name of the environment variable"),
...
var promptLogIdLiteralSchema = z10.literal("p").describe("A literal 'p' which identifies the object as a project prompt");
var promptBaseSchema = generateBaseTableSchema("prompt");
var promptSchemaObject = z10.object({
  id: promptBaseSchema.shape.id,
  // This has to be copy/pasted because zod blows up when there are circular dependencies
  _xact_id: z10.string().describe(
  ...
  slug: z10.string().describe("Unique identifier for the prompt"),
  ...
  tags: z10.array(z10.string()).nullish().describe("A list of tags for the prompt"),
...
var functionSchema2 = promptSchemaObject.merge(
  z10.object({
    ...
    origin: z10.object({
      ...
      object_id: z10.string().uuid().describe("Id of the object the function is originating from"),
      internal: z10.boolean().nullish().describe(
        "The function exists for internal purposes and should not be displayed in the list of functions."
      )
    }).nullish(),
    function_schema: z10.object({
...
var repoInfoSchema = z10.object({
  commit: z10.string().nullish().describe("SHA of most recent commit"),
  branch: z10.string().nullish().describe("Name of the branch the most recent commit belongs to"),
  tag: z10.string().nullish().describe("Name of the tag on the most recent commit"),
  dirty: z10.boolean().nullish().describe(
    "Whether or not the repo had uncommitted changes when snapshotted"
  ),
  author_name: z10.string().nullish().describe("Name of the author of the most recent commit"),
  author_email: z10.string().nullish().describe("Email of the author of the most recent commit"),
  commit_message: z10.string().nullish().describe("Most recent commit message"),
  commit_time: z10.string().nullish().describe("Time of the most recent commit"),
  git_diff: z10.string().nullish().describe(
...
var experimentSchema = z10.object({
  ...
  commit: z10.string().nullish().describe("Commit, taken directly from `repo_info.commit`"),
  base_exp_id: z10.string().uuid().nullish().describe(
    "Id of default base experiment to compare against when viewing this experiment"
  ),
  deleted_at: experimentBaseSchema.shape.deleted_at,
  dataset_id: z10.string().uuid().nullish().describe(
    "Identifier of the linked dataset, or null if the experiment is not linked to a dataset"
  ),
  dataset_version: z10.string().nullish().describe(
    "Version number of the linked dataset the experiment was run against. This can be used to reproduce the experiment after the dataset has been modified."
  ),
  public: z10.boolean().describe(
...
var promptSessionSchema = z10.object({
...
var roleSchema = z10.object({
  ...
  org_id: z10.string().uuid().nullish().describe(
    [
  ...
  member_roles: z10.array(z10.string().uuid()).nullish().describe(
    [
...
var groupSchema = z10.object({
  ...
  member_users: z10.array(z10.string().uuid()).nullish().describe("Ids of users which belong to this group"),
  member_groups: z10.array(z10.string().uuid()).nullish().describe(
    [
...
var projectScoreTypeEnum = z10.enum(["slider", "categorical", "weighted", "minimum", "online"]).describe("The type of the configured score");
var projectScoreCategory = z10.object({
  name: z10.string().describe("Name of the category"),
  value: z10.number().describe(
    "Numerical value of the category. Must be between 0 and 1, inclusive"
  )
}).describe("For categorical-type project scores, defines a single category").openapi("ProjectScoreCategory");
var onlineScoreConfigSchema = z10.object({
  sampling_rate: z10.number().min(0).max(1).describe("The sampling rate for online scoring"),
  scorers: z10.array(savedFunctionIdSchema).describe("The list of scorers to use for online scoring"),
  apply_to_root_span: z10.boolean().nullish().describe(
    "Whether to trigger online scoring on the root span of each trace"
  ),
  apply_to_span_names: z10.string().array().nullish().describe("Trigger online scoring on any spans with a name in this list")
}).refine((val) => {
  var _a2;
...
var projectScoreSchema = z10.object({
...
var projectTagSchema = z10.object({
  ...
  color: z10.string().nullish().describe("Color of the tag for the UI")
}).describe(
...
var viewSchema = z10.object({
  id: viewBaseSchema.shape.id,
  object_type: aclObjectTypeEnum,
  object_id: z10.string().uuid().describe("The id of the object the view applies to"),
...
var aclSchema = z10.object({
  id: aclBaseSchema.shape.id,
  object_type: aclObjectTypeEnum,
  object_id: z10.string().uuid().describe("The id of the object the ACL applies to"),
  user_id: z10.string().uuid().nullish().describe(
    "Id of the user the ACL applies to. Exactly one of `user_id` and `group_id` will be provided"
  ),
  group_id: z10.string().uuid().nullish().describe(
  ...
  role_id: z10.string().uuid().nullish().describe(
    "Id of the role the ACL grants. Exactly one of `permission` and `role_id` will be provided"
  ),
  _object_org_id: z10.string().uuid().describe("The organization the ACL's referred object belongs to"),
...
var appLimitSchema = z10.number().int().nonnegative().describe("Limit the number of objects to return");
function generateBaseTableOpSchema(objectName) {
  return z10.object({
    org_name: z10.string().nullish().describe(
...
var startingAfterSchema = z10.string().uuid().describe(
  [
    "Pagination cursor id.",
    "For example, if the final item in the last page you fetched had an id of `foo`, pass `starting_after=foo` to fetch the next page. Note: you may only pass one of `starting_after` and `ending_before`"
  ].join("\n\n")
).openapi("StartingAfter");
var endingBeforeSchema = z10.string().uuid().describe(
  [
    "Pagination cursor id.",
...
function makeObjectIdsFilterSchema(objectName) {
  const item = z10.string().uuid();
  return z10.union([item, item.array()]).describe(
...
var patchProjectSchema = z10.object({
...
var createExperimentSchema = z10.object({
  ...
  ensure_new: z10.boolean().nullish().describe(
...
var createDatasetSchema = z10.object({
...
var patchDatasetSchema = z10.object({
...
var patchPromptSchema = z10.object({
...
var patchFunctionSchema = z10.object({
...
var createRoleSchema = z10.object({
...
var aclBatchUpdateRequestSchema = z10.object({
  add_acls: aclItemSchema.array().nullish(),
  remove_acls: aclItemSchema.array().nullish()
});
var aclBatchUpdateResponseSchema = z10.object({
...
var createProjectTagSchema = z10.object({
...
var patchProjectTagSchema = z10.object({
...
var patchViewSchema = z10.object({
...
var deleteViewSchema = z10.object({
  object_type: viewSchema.shape.object_type,
  object_id: viewSchema.shape.object_id
}).openapi("DeleteView");
var patchOrganizationSchema = z10.object({
...
var createApiKeySchema = z10.object({
  name: z10.string().describe("Name of the api key. Does not have to be unique"),
  org_name: createApiKeyBaseSchema.shape.org_name
});
var createApiKeyOutputSchema = apiKeySchema.merge(
  z10.object({
    key: z10.string().describe("The raw API key. It will only be exposed this one time")
  })
).openapi("CreateApiKeyOutput");
var organizationMembersSchema = z10.object({
  members: userSchema.pick({ id: true, email: true }).array()
}).openapi("OrganizationMembers");
var patchOrganizationMembersSchema = z10.object({
  invite_users: z10.object({
    ...
    send_invite_emails: z10.boolean().nullish().describe(
  ...
  org_id: z10.string().nullish().describe(
...
var patchOrganizationMembersOutputSchema = z10.object({
  status: z10.literal("success"),
  send_email_error: z10.string().nullish().describe(
...
var deleteAISecretSchema = z10.object({
  name: aiSecretSchema.shape.name,
  org_name: createAISecretBaseSchema.shape.org_name
});
var patchAISecretSchema = z10.object({
  name: aiSecretSchema.shape.name.nullish(),
  type: aiSecretSchema.shape.type,
  metadata: aiSecretSchema.shape.metadata,
  secret: z10.string().nullish()
});
var createEnvVarSchema = envVarSchema.pick({ object_type: true, object_id: true, name: true }).extend({
  value: z10.string().nullish().describe(
...
// typespecs/api_types.ts
extendZodWithOpenApi4(z11);
var auditSourcesSchema = z11.enum(VALID_SOURCES);
function generateBaseEventOpSchema(objectType) {
  ...
  return z11.object({
    ...
    [TRANSACTION_ID_FIELD]: z11.string().describe(
    ...
    context: z11.object({
      caller_functionname: z11.string().nullish().describe(
        `The function in code which created the ${eventDescription} event`
      ),
      caller_filename: z11.string().nullish().describe(
        `Name of the file in code where the ${eventDescription} event was created`
      ),
      caller_lineno: z11.number().int().nullish().describe(
    ...
    span_parents: z11.string().array().nullish().describe(
    ...
    root_span_id: z11.string().describe(
      `The \`span_id\` of the root of the trace this ${eventDescription} event belongs to`
    ),
    span_attributes: z11.object({
      name: z11.string().nullish().describe("Name of the span, for display purposes only"),
      type: z11.enum(spanTypeAttributeValues).nullish().describe("Type of the span, for display purposes only")
    }).catchall(customTypes.unknown).nullish().describe(
      "Human-identifying attributes of the span, such as name, type, etc."
    ),
    [OBJECT_DELETE_FIELD]: z11.boolean().nullish().describe(
...
function generateBaseEventFeedbackSchema(objectType) {
  ...
  return z11.object({
    ...
    scores: z11.record(z11.number().min(0).max(1).nullish()).nullish().describe(
    ...
    tags: z11.array(z11.string()).nullish().describe("A list of tags to log"),
    comment: z11.string().nullish().describe(
...
var fetchLimitSchema = z11.number().int().nonnegative().describe(
  [
...
var maxXactIdSchema = z11.string().describe(fetchPaginationCursorDescription);
var maxRootSpanIdSchema = z11.string().describe(fetchPaginationCursorDescription);
var fetchPaginationCursorSchema = z11.string().describe(
  [
...
var versionSchema = z11.string().describe(
  [
...
var pathTypeFilterSchema = z11.object({
  type: z11.literal("path_lookup").describe("Denotes the type of filter as a path-lookup filter"),
  path: z11.string().array().describe(
...
var fetchEventsRequestSchema = z11.object({
...
function makeFetchEventsResponseSchema(objectType, eventSchema) {
  ...
  return z11.object({
    ...
    cursor: z11.string().nullish().describe(
      [
        "Pagination cursor",
...
var experimentEventSchema = z11.object({
  id: experimentEventBaseSchema.shape.id,
  dataset_record_id: z11.string().nullish().describe(
...
var datasetEventSchema = z11.object({
...
var promptSessionEventSchema = z11.object({
...
var projectLogsLogIdLiteralSchema = z11.literal("g").describe("A literal 'g' which identifies the log as a project log");
var projectLogsEventSchema = z11.object({
...
var mergeEventSchema = z11.object({
  [IS_MERGE_FIELD]: customTypes.literalTrue.describe(isMergeDescription),
  [MERGE_PATHS_FIELD]: z11.string().array().array().nullish().describe(
    [
...
var replacementEventSchema = z11.object({
  [IS_MERGE_FIELD]: customTypes.literalFalse.nullish().describe(isMergeDescription),
  [PARENT_ID_FIELD]: z11.string().nullish().describe(
    [
...
function makeInsertEventSchemas(objectType, insertSchema) {
  ...
  const eventSchema = z11.union([replaceVariantSchema, mergeVariantSchema]).describe(`${capitalize(article)} ${eventDescription} event`).openapi(`Insert${eventSchemaName}Event`);
  const requestSchema = z11.object({
...
function makeCrossObjectIndividualRequestSchema(objectType) {
  ...
  return z11.record(z11.string().uuid(), insertObject).nullish().describe(
...
function makeCrossObjectIndividualResponseSchema(objectType) {
  return z11.record(z11.string().uuid(), insertEventsResponseSchema).nullish().describe(
    `A mapping from ${objectType} id to row ids for inserted \`events\``
  );
}
var crossObjectInsertRequestSchema = z11.object({
...
var crossObjectInsertResponseSchema = z11.object({
...
var summarizeScoresParamSchema = z11.boolean().describe(
  "Whether to summarize the scores and metrics. If false (or omitted), only the metadata will be returned."
);
var comparisonExperimentIdParamSchema = z11.string().uuid().describe(
...
var summarizeDataParamSchema = z11.boolean().describe(
  "Whether to summarize the data. If false (or omitted), only the metadata will be returned."
);
var summarizeExperimentResponseSchema = z11.object({
  project_name: z11.string().describe("Name of the project that the experiment belongs to"),
  experiment_name: z11.string().describe("Name of the experiment"),
  project_url: z11.string().url().describe("URL to the project's page in the Braintrust app"),
  experiment_url: z11.string().url().describe("URL to the experiment's page in the Braintrust app"),
  comparison_experiment_name: z11.string().nullish().describe("The experiment which scores are baselined against"),
  scores: z11.record(
    z11.object({
      name: z11.string().describe("Name of the score"),
      score: z11.number().min(0).max(1).describe("Average score across all examples"),
      diff: z11.number().min(-1).max(1).optional().describe(
        "Difference in score between the current and comparison experiment"
      ),
      improvements: z11.number().int().min(0).describe("Number of improvements in the score"),
      regressions: z11.number().int().min(0).describe("Number of regressions in the score")
    }).describe("Summary of a score's performance").openapi("ScoreSummary")
  ).nullish().describe("Summary of the experiment's scores"),
  metrics: z11.record(
    z11.object({
      name: z11.string().describe("Name of the metric"),
      metric: z11.number().describe("Average metric across all examples"),
      unit: z11.string().describe("Unit label for the metric"),
      diff: z11.number().optional().describe(
        "Difference in metric between the current and comparison experiment"
      ),
      improvements: z11.number().int().min(0).describe("Number of improvements in the metric"),
      regressions: z11.number().int().min(0).describe("Number of regressions in the metric")
...
var summarizeDatasetResponseSchema = z11.object({
  project_name: z11.string().describe("Name of the project that the dataset belongs to"),
  dataset_name: z11.string().describe("Name of the dataset"),
  project_url: z11.string().url().describe("URL to the project's page in the Braintrust app"),
  dataset_url: z11.string().url().describe("URL to the dataset's page in the Braintrust app"),
  data_summary: z11.object({
    total_records: z11.number().int().min(0).describe("Total number of records in the dataset")
...
var asyncScoringStateSchema = z11.union([
  z11.object({
    status: z11.literal("enabled"),
    token: z11.string(),
    function_ids: z11.array(functionIdSchema).nonempty()
  }),
  // Explicitly disabled.
  z11.object({
    status: z11.literal("disabled")
  }),
  // Inactive but may be selected later.
  z11.null()
]);
var asyncScoringControlSchema = z11.discriminatedUnion("kind", [
  z11.object({
    kind: z11.literal("score_update"),
    token: z11.string()
  }),
  z11.object({
    kind: z11.literal("state_override"),
    state: asyncScoringStateSchema
  }),
  z11.object({
    kind: z11.literal("state_force_reselect")
  })
]);
export {
  BRAINTRUST_PARAMS,
