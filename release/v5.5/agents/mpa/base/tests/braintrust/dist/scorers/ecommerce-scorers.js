 ...
 * @returns TurnScore
 */
export function scoreRfmSegmentRecognition(output, input) {
    // Check if user mentioned customer segments or RFM-like data
    const userMentionsSegments = /customers?\s*(who|that|haven't|didn't)/i.test(input) ||
    ...
    const hasSegmentStrategy = /target(ing)?\s*(these|those|this|the)\s*(segment|customers?|group)/i.test(output) ||
 ...
 * @returns TurnScore
 */
export function scoreReactivationStrategy(output, input) {
    // Check if user asks about dormant/inactive customers
    const asksAboutDormant = /dormant|inactive|lapsed|haven't\s*(purchased|bought|ordered)/i.test(input) ||
    ...
    const hasSubSegmentation = /(recently|long[\s-]?term)\s*(dormant|lapsed|inactive)/i.test(output) ||
 ...
 * @returns TurnScore
 */
export function scoreRetentionAcquisitionBalance(output, input) {
    // Check if user provides new vs returning data
    const hasNewVsReturning = /(new|returning|repeat)\s*(customers?|users?|revenue)/i.test(input) ||
        /\d+%.*\d+%/i.test(input) ||
 ...
 * @returns TurnScore
 */
export function scoreCohortAnalysisUsage(output, input) {
    // Check if user provides retention/cohort data
    const hasCohortData = /retention.*\d+%/i.test(input) ||
    ...
    const hasActionableResponse = /to\s*(improve|address|reverse|combat)/i.test(output) ||
 ...
 * @returns TurnScore
 */
export function scoreLookalikeAudienceStrategy(output, input) {
    // Check if user mentions VIP/high-value customers
    const hasHighValueMention = /vip/i.test(input) ||
    ...
    const hasSeedQuality = /quality|size|minimum|at\s*least/i.test(output) &&
 ...
 * @returns TurnScore
 */
export function scoreSeasonalPlanning(output, input) {
    ...
    const hasStrategyAdjustment = /adjust|shift|increase|decrease|reallocate/i.test(output) ||
 ...
 * @returns TurnScore
 */
export function scoreCustomerLtvApplication(output, input) {
    // Check if user provides LTV or value data
    const hasLtvData = LTV_PATTERNS.some(p => p.test(input)) ||
    ...
    const hasCacLtvRatio = /cac.*ltv|ltv.*cac/i.test(output) ||
        /ratio/i.test(output) ||
        /\d+:\d+/i.test(output) ||
        /\d+x/i.test(output);
    // Scoring logic
 ...
 * @returns TurnScore
 */
export function scoreRetentionChannelMix(output, input) {
    // Check if context is about retention
    const isRetentionContext = /retention|retain|returning|repeat|loyal|dormant|re-?activat/i.test(input);
    ...
    const hasLifecycleEmphasis = /lifecycle|automation|flow|sequence|series/i.test(output);
    // Check for segmented approach
    const hasSegmentedApproach = /segment|tier|different\s*(message|approach|content)/i.test(output);
    // Scoring logic
    let score = 0;
    let status = 'needs_improvement';
...
//# sourceMappingURL=ecommerce-scorers.js.map