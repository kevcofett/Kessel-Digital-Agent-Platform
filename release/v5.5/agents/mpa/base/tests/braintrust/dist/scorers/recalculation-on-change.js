 ...
 */
export function detectDataChange(priorData, currentUserMessage) {
    ...
    for (const pattern of budgetPatterns) {
        ...
        if (match) {
            ...
            const oldValue = priorData.budget;
    ...
    for (const pattern of volumePatterns) {
        ...
        if (match) {
            const newValue = parseNumericValue(match[1]);
            const oldValue = (priorData.volumeTarget || priorData.customers || priorData.conversions);
            if (oldValue && Math.abs(newValue - oldValue) / oldValue > 0.05) {
                return {
                    field: 'volumeTarget',
                    oldValue,
 ...
 */
function hasRecalculation(response) {
 ...
 */
function justAcknowledges(response) {
 ...
 */
export async function scoreRecalculationOnChange(ctx) {
...
//# sourceMappingURL=recalculation-on-change.js.map