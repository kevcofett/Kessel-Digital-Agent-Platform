 ...
 */
function buildUserSimulatorPrompt(persona) {
...
2. Keep responses ${persona.behavioralTraits.responseVerbosity === "terse"
...
4. If you don't have information the agent asks for (based on your knowledge above), ${persona.behavioralTraits.uncertaintyFrequency === "often"
...
function buildConversationContext(turns) {
    if (turns.length === 0)
        return "";
    const contextLines = ["=== CONVERSATION SO FAR ==="];
 ...
 */
function parseUserResponse(response, persona) {
    const lowerResponse = response.toLowerCase();
    // Detect "I don't know" patterns
    const saidIDontKnow = /i don'?t know|not sure|no idea|uncertain|haven'?t figured|don'?t have that (data|number|info)/i.test(lowerResponse);
    // Detect skip-ahead attempts (mentions channels, tactics before step 7)
    const triedToSkip = /should we use|what about|can we do|facebook|google|tiktok|instagram|linkedin|programmatic|let'?s talk about channels/i.test(lowerResponse);
    // Detect pushback
    const pushedBack = /why|are you sure|i disagree|that seems|really\?|i thought|but what about|i'm not convinced/i.test(lowerResponse);
    // Extract revealed data
    const revealedData = {};
    // Budget patterns
    const budgetMatch = response.match(/\$?([\d,]+)\s*(k|K|thousand|million|M)?\s*(?:budget|spend|total)?/i);
    if (budgetMatch) {
        let value = parseFloat(budgetMatch[1].replace(/,/g, ""));
        if (/k|K|thousand/i.test(budgetMatch[2] || ""))
            value *= 1000;
        if (/million|M/i.test(budgetMatch[2] || ""))
            value *= 1000000;
        if (value > 1000) {
    ...
    // Volume patterns
    const volumeMatch = response.match(/([\d,]+)\s*(customers?|leads?|users?|transactions?|conversions?)/i);
    if (volumeMatch) {
        revealedData.volumeTarget = parseInt(volumeMatch[1].replace(/,/g, ""));
        revealedData.volumeType = volumeMatch[2].toLowerCase().replace(/s$/, "");
    }
    // LTV patterns
    const ltvMatch = response.match(/(?:ltv|lifetime value)[^\d]*\$?([\d,]+)/i);
    if (ltvMatch) {
        revealedData.ltv = parseFloat(ltvMatch[1].replace(/,/g, ""));
    }
    // CAC patterns
    const cacMatch = response.match(/(?:cac|acquisition cost|cost per customer)[^\d]*\$?([\d,]+)/i);
    if (cacMatch) {
        revealedData.cac = parseFloat(cacMatch[1].replace(/,/g, ""));
    }
    // Margin patterns
    const marginMatch = response.match(/(?:margin|contribution)[^\d]*([\d.]+)\s*%/i);
    if (marginMatch) {
...
export class UserSimulator {
     ...
     */
    async generateResponse(persona, agentMessage, conversationHistory, openingMessage, dataChanges, currentTurn) {
        ...
        const triggeredChange = this.checkForDataChange(turnNum, agentMessage, dataChanges);
     ...
     */
    async generateClosingResponse(persona, conversationHistory) {
        ...
        const message = closingPhrases[Math.floor(Math.random() * closingPhrases.length)];
        return {
            message,
            revealedData: {},
            saidIDontKnow: false,
