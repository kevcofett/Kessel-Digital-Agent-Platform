...
export interface KBDocument {
  ...
  step: number | number[]; // Which step(s) this document is injected for
  version: string;
  lastModified: string;
...
export interface KBUsageRecord {
  ...
  step: number;
  wasInjected: boolean;
  wasReferenced: boolean; // Did agent mention/cite this document's content?
  contentMatches: string[]; // Specific content from KB that appeared in response
  turnNumber: number;
}
 ...
 */
export const MPA_KB_DOCUMENTS: KBDocument[] = [
  {
 ...
 */
const FILENAME_TO_ID_MAP: Record<string, string> = {};
 ...
 */
const KB_REFERENCE_PATTERNS: Record<string, RegExp[]> = {
 ...
 */
export function trackKBUsage(
  agentResponse: string,
  injectedDocuments: string[],
  step: number,
  turnNumber: number
): KBUsageRecord[] {
  const records: KBUsageRecord[] = [];
 ...
 */
export function calculateKBImpactMetrics(
  usageRecords: KBUsageRecord[],
  qualityScores: {
    turnNumber: number;
    mentorship: number;
    citation: number;
  ...
  const metrics: Record<string, KBImpactMetrics> = {};
  ...
  for (const docId of Object.keys(metrics)) {
    ...
    const scoresWhenInjected = qualityScores.filter((s) =>
      s.injectedDocuments.includes(docId)
    );
    const scoresWhenNotInjected = qualityScores.filter(
      (s) => !s.injectedDocuments.includes(docId)
    );
 ...
 */
export function generateKBOptimizationRecommendations(
  impactMetrics: KBImpactMetrics[]
): KBOptimizationRecommendation[] {
  const recommendations: KBOptimizationRecommendation[] = [];
 ...
 */
export function generateKBImpactReport(
  impactMetrics: KBImpactMetrics[],
  ...
  for (const metric of impactMetrics.sort((a, b) => b.mentorshipImpact - a.mentorshipImpact)) {
    lines.push(
      `| ${metric.documentName.slice(0, 30)} | ${metric.timesInjected} | ${metric.timesReferenced} | ${(metric.referenceRate * 100).toFixed(0)}% | ${formatDelta(metric.mentorshipImpact)} | ${formatDelta(metric.citationImpact)} |`
    );
  }
  lines.push("");
 ...
 */
export function saveKBImpactData(
  impactMetrics: KBImpactMetrics[],
