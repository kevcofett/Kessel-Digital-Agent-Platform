 ...
 */
export const multiAudienceUnifiedPersona: UserPersona = {
 ...
 */
export const multiAudienceUnifiedPlanScenario: TestScenario = {
  id: "multi-audience-unified-plan",
  name: "Multi-Audience Unified Media Plan",
  category: "phase1-quality",
  description:
    "Tests agent's ability to handle multiple distinct target audiences with " +
  ...
  openingMessage:
    "Hi, I'm the Director of Marketing at TrueHealth Insurance. We need to build a " +
  ...
  failureConditions: [
    {
      ...
      description:
        "Agent merges or confuses attributes between distinct segments",
      type: "custom_pattern",
      severity: "critical",
      scorePenalty: 0.2,
    ...
    },
    {
      ...
      description:
        "Agent treats multi-segment plan as single homogeneous audience",
      type: "custom_pattern",
      severity: "major",
      scorePenalty: 0.15,
      terminateOnDetect: false,
    },
    {
      ...
      description:
        "Agent fails to define or acknowledge one of the three segments",
      type: "custom_pattern",
      severity: "major",
      scorePenalty: 0.15,
      terminateOnDetect: false,
    },
    {
      ...
      description:
        "Agent doesn't discuss how to allocate budget across segments",
      type: "custom_pattern",
      severity: "warning",
      scorePenalty: 0.1,
      terminateOnDetect: false,
    ...
    customCriteria: [
      {
        ...
        evaluator: (result) => {
          const allResponses = result.turns
            ...
            .join(" ");
          const segment1 = /(?:young professional|26.35|urban|gym|digital.first)/i.test(
            allResponses
          );
          const segment2 = /(?:famil|35.50|suburban|parent|kid|children)/i.test(
            allResponses
          );
          const segment3 = /(?:pre.medicare|55.64|retirement|florida|arizona)/i.test(
            allResponses
          );
      ...
      },
      {
        ...
        evaluator: (result) => {
          ...
          const urbanGeo = /(?:young|26.35).*(?:urban|nyc|sf|chicago)/i.test(
            allResponses
          );
          const suburbanGeo = /(?:famil|parent).*(?:suburban|ring)/i.test(
            allResponses
          );
          const retirementGeo = /(?:pre.medicare|55.64|retire).*(?:florida|arizona|texas)/i.test(
            allResponses
          );
      ...
      },
      {
        ...
        evaluator: (result) => {
          ...
          const youngBehaviors = /(?:health.conscious|gym|fitness|digital|convenience)/i.test(
            allResponses
          );
          const familyBehaviors = /(?:coverage|cost|predicta|child|dependent)/i.test(
            allResponses
          );
          const premedBehaviors = /(?:stability|network|medicare|transition|retire)/i.test(
            allResponses
          );
      ...
      },
      {
        ...
        evaluator: (result) => {
          ...
          const confusionFound = confusedPatterns.filter((p) =>
            p.test(allResponses)
          ).length;
