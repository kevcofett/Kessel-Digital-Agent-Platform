 ...
 */
export const sophisticatedUserPersona: UserPersona = {
 ...
 */
export const sophisticatedIdkScenario: TestScenario = {
  id: "sophisticated-idk-protocol",
  name: "Sophisticated User - IDK Protocol",
  description:
    "Tests agent's ability to handle uncertainty, model with assumptions, and continue progress when user says 'I don't know'",

  persona: sophisticatedUserPersona,

  openingMessage:
    "Hey, I'm the growth lead at QuickSend. We're a remittance fintech looking to acquire 10,000 new users this quarter with a $500K media spend. Let's build out the media strategy.",
  ...
  failureConditions: [
    ...
    },
    {
      ...
      type: "custom_pattern",
      detectionPattern:
        "in simple terms|to put it simply|basically what this means",
      severity: "warning",
      scorePenalty: 0.05,
      terminateOnDetect: false,
    },
    ...
    customCriteria: [
      {
        ...
        evaluator: (result) => {
          const idkEvents = result.allEvents.filter(
            (e) => e.type === "idk_protocol"
          );
          if (idkEvents.length === 0) return true; // No IDK events

          // Check that benchmark was cited after IDK
          const benchmarkCited = result.allEvents.some(
            (e) =>
              e.type === "benchmark_cited" &&
      ...
      },
      {
        ...
        description:
          "Agent calculates CAC from budget/volume without asking for CAC",
        evaluator: (result) => {
          // Check that agent calculates $500K / 10K = $50 CAC
          const calculationPerformed = result.turns.some((turn) =>
            /\$50|\$50\.00|50 per|50\/user|50 per user/i.test(turn.agentResponse)
          );
      ...
      },
      {
        ...
        evaluator: (result) => {
          return result.turns.some((turn) =>
            /remittance|fintech|money transfer|transaction|take rate/i.test(
              turn.agentResponse
            )
          );
        },
