 ...
 */
export function scoreAdisTriggerDetection(
  response: string,
  userMessage: string
): TurnScore {
 ...
 */
export function scoreRfmRecognition(
  response: string,
  userMessage: string
): TurnScore {
  const rfmKeywords = ["rfm", "recency", "frequency", "monetary", "best customers"];
  const hasRfmRequest = rfmKeywords.some((kw) =>
    userMessage.toLowerCase().includes(kw)
  );
  ...
  const acknowledged = rfmAcknowledgment.some((pattern) =>
 ...
 */
export function scoreClvRecognition(
  response: string,
  userMessage: string
): TurnScore {
  const clvKeywords = [
  ...
  const hasClvRequest = clvKeywords.some((kw) =>
    userMessage.toLowerCase().includes(kw)
  );
  ...
  const acknowledged = clvAcknowledgment.some((pattern) =>
    pattern.test(response)
  );
 ...
 */
export function scoreAmmoTrigger(
  response: string,
  conversationContext: { hasAudiences: boolean; hasBudget: boolean }
): TurnScore {
  ...
  const offeredAmmo = ammoOfferPatterns.some((pattern) =>
    pattern.test(response)
  );
 ...
 */
export function scoreAllocationRationale(response: string): TurnScore {
  let score = 0;
  ...
  if (
    /expected.*conversion/i.test(response) ||
 ...
 */
export function scoreSegmentPresentation(response: string): TurnScore {
  let score = 0;
 ...
 */
export function scoreChannelAffinityRecommendation(response: string): TurnScore {
  let score = 0;
 ...
 */
export function scoreJargonAvoidance(
  response: string,
  ...
  for (const term of jargonTerms) {
    if (response.toUpperCase().includes(term.toUpperCase())) {
      ...
      const sentenceWithTerm = sentences.find((s) =>
        s.toUpperCase().includes(term.toUpperCase())
      );
 ...
 */
export function scoreResultSimplification(
  response: string,
  userSophistication: "basic" | "intermediate" | "advanced" | "expert"
): TurnScore {
  if (userSophistication !== "basic") {
  ...
  const segmentsMentioned = allSegments.filter((s) =>
 ...
 */
export function scoreAdisIntegration(
  response: string,
  context: {
    userMessage: string;
