...
export interface CrossStepSynthesisContext {
  ...
  currentStep: number;
  stepSummaries: {
    step1?: string;  // Outcomes
    step2?: string;  // Economics
    step3?: string;  // Audience
    step4?: string;  // Geography
  };
  previousQuestions?: string[];
}

// Expected synthesis connections by step
const EXPECTED_REFERENCES: Record<number, number[]> = {
 ...
 */
function hasExplicitStepReferences(response: string): {
  hasExplicit: boolean;
  ...
  const implicitReferences: Record<number, RegExp[]> = {
 ...
 */
export async function scoreCrossStepSynthesis(
  ctx: CrossStepSynthesisContext
): Promise<JudgeResult> {
  ...
  const matchedRefs = expectedRefs.filter(step =>
    references.referencedSteps.includes(step)
  );
