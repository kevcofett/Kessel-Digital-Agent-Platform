...
export interface IndustryBenchmarks {
  ...
  typicalChannelMix: Record<string, { min: number; max: number }>;
 ...
 */
function checkBudgetSums(
  channelAllocations: Record<string, number> | undefined,
  ...
  const allocatedTotal = Object.values(channelAllocations).reduce(
    (sum, val) => sum + val,
    0
  );
 ...
 */
function checkGeographySpend(
  geoAllocations: Record<string, number> | undefined,
  totalBudget: number | undefined
): { passed: boolean; details: string } {
  ...
  const geoTotal = Object.values(geoAllocations).reduce(
    (sum, val) => sum + val,
    0
  );
 ...
 */
function checkTestBudgetCarveout(
  testBudget: number | undefined,
  totalBudget: number | undefined
): { passed: boolean; details: string } {
 ...
 */
function checkVolumeProjections(
  projectedVolume: number | undefined,
  impliedCAC: number | undefined,
  totalBudget: number | undefined
): { passed: boolean; details: string } {
 ...
 */
function checkChannelMixPercentages(
  channelMix: Record<string, number> | undefined
): { passed: boolean; details: string } {
  if (!channelMix) {
 ...
 */
export function scoreMathematicalConsistency(plan: MediaPlan): TurnScore {
  const checks = [
 ...
 */
export async function scoreStrategicCoherence(plan: MediaPlan): Promise<TurnScore> {
  const prompt = STRATEGIC_COHERENCE_JUDGE_PROMPT.replace(
    "{objective}",
    plan.objective || "Not defined"
  )
    ...
    .replace(
      "{audience}",
      plan.audienceProfile?.demographics || "Not defined"
    )
    .replace("{geography}", plan.geoScope || "Not defined")
    .replace(
      "{channels}",
      plan.channelMix ? Object.keys(plan.channelMix).join(", ") : "Not defined"
    )
 ...
 */
export async function scoreDefensibility(
  plan: MediaPlan,
  benchmarks: IndustryBenchmarks = DEFAULT_BENCHMARKS
): Promise<TurnScore> {
  const prompt = DEFENSIBILITY_JUDGE_PROMPT.replace(
    "{budget}",
    String(plan.totalBudget || "Not defined")
  )
    .replace("{volume_target}", String(plan.volumeTarget || "Not defined"))
    .replace("{volume_unit}", plan.volumeUnit || "customers")
    .replace("{implied_cac}", String(plan.impliedCAC || "Not calculated"))
 ...
 */
export function scorePlanComprehensiveness(plan: MediaPlan): TurnScore {
 ...
 */
export function scoreEfficiencyRealism(
  plan: MediaPlan,
  benchmarks: IndustryBenchmarks = DEFAULT_BENCHMARKS
): TurnScore {
  if (!plan.impliedCAC) {
 ...
 */
export function calculatePlanCoherenceScore(
 ...
 */
export async function scorePlanCoherence(
  plan: MediaPlan,
  benchmarks: IndustryBenchmarks = DEFAULT_BENCHMARKS
