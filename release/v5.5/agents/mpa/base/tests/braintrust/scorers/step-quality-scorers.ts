 ...
 */
const STEP_BASE_REQUIREMENTS: Record<
  number,
  { minDataPoints: number; requiresCalculation: boolean; minTurns: number }
> = {
 ...
 */
const FUNNEL_STEP_WEIGHTS: Record<string, Record<number, number>> = {
  awareness: {
 ...
 */
function getBudgetMultiplier(budget: number): number {
 ...
 */
function getAggressivenessMultiplier(
...
export function getStepRequirements(
  ...
  const aggressivenessMultiplier = getAggressivenessMultiplier(
    context.kpiAggressiveness
  );

  const combinedMultiplier =
    budgetMultiplier * funnelMultiplier * aggressivenessMultiplier;

  return {
    minDataPoints: Math.ceil(baseReq.minDataPoints * combinedMultiplier),
    requiresCalculation:
      baseReq.requiresCalculation && context.funnel === "performance",
...
const STEP_QUALITY_INDICATORS: Record<number, string[]> = {
  1: [
...
export async function scoreStepQuality(
  input: StepQualityInput,
  context: ScenarioContext
): Promise<TurnScore> {
  const requirements = getStepRequirements(input.stepNumber, context);

  const prompt = STEP_DEPTH_JUDGE_PROMPT.replace(
    "{step_number}",
    ...
    .replace(
      "{quality_indicators}",
      STEP_QUALITY_INDICATORS[input.stepNumber]?.join("\n- ") || "None defined"
    )
 ...
 */
export function scoreStepDataCompleteness(
  input: StepQualityInput,
  context: ScenarioContext
): TurnScore {
  const requirements = getStepRequirements(input.stepNumber, context);
  const dataPointRatio =
    input.dataPointsCollected.length / requirements.minDataPoints;
  ...
  if (
    requirements.requiresCalculation &&
 ...
 */
export function scoreStepTurnEfficiency(
  input: StepQualityInput,
  context: ScenarioContext
): TurnScore {
  const requirements = getStepRequirements(input.stepNumber, context);
  ...
  return {
    ...
    metadata: {
      ...
      turnRatio,
      dataPointsPerTurn:
        input.dataPointsCollected.length / Math.max(1, input.turnsInStep),
    },
    scope: "turn",
  };
}
 ...
 */
export function scoreStepSynthesis(
  input: StepQualityInput,
  priorStepInsights: Record<number, string[]>
): TurnScore {
  ...
  const synthesisPatterns: Record<number, RegExp[]> = {
    5: [
  ...
  const hasExplicitReference = explicitReferences.some((p) =>
    p.test(allResponses)
  );
 ...
 */
export function calculateStepQualityScore(
