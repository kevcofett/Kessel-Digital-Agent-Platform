 ...
 */
export async function scoreAudienceCompleteness(
  output: string,
  currentStep: number,
  cacAggressiveness: "aggressive" | "moderate" | "conservative" | "unknown"
): Promise<TurnScore> {
 ...
 */
export function scoreAudienceSizing(
  output: string,
  currentStep: number
): TurnScore {
  ...
  if (hasTable) score += 0.15;
  if (hasGeoColumn) score += 0.15;
  if (hasPopulationColumn) score += 0.15;
  if (hasTargetAudienceColumn) score += 0.15;
  if (hasPercentColumn) score += 0.1;
  if (hasAbsoluteNumbers) score += 0.15;
  if (hasTotalRow) score += 0.1;
  if (hasMultipleDMAs) score += 0.05;
 ...
 */
export async function scorePrecisionConnection(
  output: string,
  currentStep: number
): Promise<TurnScore> {
 ...
 */
export function scoreResponseFormatting(output: string): TurnScore {
 ...
 */
export function scoreMathAccuracy(
  output: string,
  ...
  while ((match = tableRowPattern.exec(output)) !== null) {
    ...
    if (population && targetAudience && !isNaN(statedPercent)) {
      ...
      if (Math.abs(calculatedPercent - statedPercent) > tolerance) {
        errors.push(
          `Table math error: ${targetAudience} / ${population} = ${calculatedPercent.toFixed(1)}%, stated ${statedPercent}%`
        );
      } else {
  ...
  while ((match = cacPattern.exec(output)) !== null) {
    ...
    if (budget && customers && !isNaN(statedCac)) {
      ...
      if (Math.abs(calculatedCac - statedCac) > tolerance) {
        errors.push(
          `CAC math error: $${budget} / ${customers} = $${calculatedCac.toFixed(0)}, stated $${statedCac}`
        );
      } else {
  ...
  while ((match = arithmeticPattern.exec(output)) !== null) {
    ...
    if (operand1 && operand2 && statedResult) {
      ...
      if (Math.abs(calculatedResult - statedResult) > Math.max(tolerance, 1)) {
        errors.push(
          `Arithmetic error: ${operand1} / ${operand2} = ${calculatedResult.toFixed(1)}, stated ${statedResult}`
        );
      } else {
 ...
 */
export function scoreFeasibilityValidation(
  output: string,
  ...
  const budget = extractedData.budget as number | undefined;
  const customerTarget = extractedData.customerTarget as number | undefined;
  const audienceSize = extractedData.audienceSize as number | undefined;
  const statedCac = extractedData.cac as number | undefined;
  ...
  if (audienceSize && customerTarget) {
    ...
    if (requiredConversionRate > 0.05) {
      issues.push(
        `Unrealistic conversion: ${customerTarget} from ${audienceSize} audience requires ${(requiredConversionRate * 100).toFixed(1)}% conversion (>5% is aggressive for cold traffic)`
      );
    } else if (requiredConversionRate > 0.02) {
      validations.push(
        `Aggressive but possible: ${(requiredConversionRate * 100).toFixed(1)}% conversion required`
      );
    } else {
      validations.push(
        `Feasible conversion rate: ${(requiredConversionRate * 100).toFixed(2)}%`
      );
    }
  }
  ...
  if (audienceSize && budget) {
    ...
    if (budgetPerPerson < 0.5) {
      issues.push(
        `Low investment density: $${budgetPerPerson.toFixed(2)} per person may be insufficient for conversion`
      );
    } else {
  ...
  if (audienceSize && customerTarget) {
    ...
    if (penetrationRate > 0.1) {
      issues.push(
        `High penetration risk: ${(penetrationRate * 100).toFixed(1)}% of audience must convert - saturation likely`
      );
    }
  }
 ...
 */
export function scoreBenchmarkSourcing(
  output: string,
  kbContent?: string
): TurnScore {
...
export async function scoreTurn(
  ...
  const [
    ...
    progressScore,
    riskScore,
    audienceCompletenessScore,
    precisionConnectionScore,
  ] = await Promise.all([
