...
class FlowValidator:
    ...
    def _check_column_names(self, flow: dict) -> List[str]:
        ...
        if "mpa_isactive" in flow_str:
            issues.append("CRITICAL: Found 'mpa_isactive' (should be 'mpa_isactive')")
        if "mpa_newcolumn" in flow_str:
            issues.append("CRITICAL: Found 'mpa_newcolumn' (should be 'mpa_newcolumn')")
