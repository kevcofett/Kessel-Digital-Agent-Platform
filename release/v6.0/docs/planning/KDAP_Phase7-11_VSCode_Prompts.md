# KDAP Phase 7-11: VS Code Prompts
...
### Prompt 7.1: Deploy CA Dataverse Tables
```
...
1. ca_framework (from base/dataverse/schema/ca_framework.json)
   - 60 framework reference records
   - Lookup relationships to mpa_vertical (for industry mapping)
...
3. ca_deliverable
   CREATE TABLE ca_deliverable:
   - ca_deliverable_id (PK, GUID, auto-generated)
   - ca_project_id (Lookup → ca_project, Required)
...
### Prompt 7.2: Load ca_framework Seed Data
```
TASK: Load 60 Framework Records into ca_framework Table

SOURCE: base/dataverse/seed/ca_framework_seed.csv
...
### Prompt 7.3: Add CST Capabilities to eap_capability
```
...
### Prompt 7.4: Add CHG Capabilities to eap_capability
```
...
### Prompt 7.5: Add ANL Financial Capabilities
```
...
### Prompt 7.6: Add DOC Consulting Capabilities
```
...
### Prompt 8.1: Create CST AI Builder Prompts
```
...
### Prompt 8.2: Create CHG AI Builder Prompts
```
...
### Prompt 8.3: Create ANL Financial Prompts
```
...
### Prompt 8.4: Create DOC Consulting Prompts
```
...
### Prompt 8.5: Register All Prompts in eap_prompt
```
...
CST Prompts (4):
- CST_FRAMEWORK_SELECT, Select Consulting Framework, CST, base/platform/eap/prompts/CST_FRAMEWORK_SELECT_PROMPT.json
- CST_ENGAGEMENT_GUIDE, Guide Consulting Engagement, CST, base/platform/eap/prompts/CST_ENGAGEMENT_GUIDE_PROMPT.json
- CST_STRATEGIC_ANALYZE, Apply Strategic Analysis, CST, base/platform/eap/prompts/CST_STRATEGIC_ANALYZE_PROMPT.json
- CST_PRIORITIZE, Prioritize Initiatives, CST, base/platform/eap/prompts/CST_PRIORITIZE_PROMPT.json

CHG Prompts (3):
- CHG_READINESS, Assess Change Readiness, CHG, base/platform/eap/prompts/CHG_READINESS_PROMPT.json
- CHG_STAKEHOLDER, Map Stakeholders, CHG, base/platform/eap/prompts/CHG_STAKEHOLDER_PROMPT.json
- CHG_ADOPTION, Plan Adoption, CHG, base/platform/eap/prompts/CHG_ADOPTION_PROMPT.json

ANL Financial Prompts (6):
- ANL_NPV, Calculate NPV, ANL, base/platform/eap/prompts/ANL_NPV_PROMPT.json
- ANL_IRR, Calculate IRR, ANL, base/platform/eap/prompts/ANL_IRR_PROMPT.json
- ANL_TCO, Calculate TCO, ANL, base/platform/eap/prompts/ANL_TCO_PROMPT.json
- ANL_MONTECARLO, Monte Carlo Simulation, ANL, base/platform/eap/prompts/ANL_MONTECARLO_PROMPT.json
- ANL_SENSITIVITY, Sensitivity Analysis, ANL, base/platform/eap/prompts/ANL_SENSITIVITY_PROMPT.json
- ANL_PAYBACK, Calculate Payback, ANL, base/platform/eap/prompts/ANL_PAYBACK_PROMPT.json

DOC Consulting Prompts (2):
- DOC_BUSINESSCASE, Generate Business Case, DOC, base/platform/eap/prompts/DOC_BUSINESSCASE_PROMPT.json
- DOC_ROADMAP, Generate Roadmap, DOC, base/platform/eap/prompts/DOC_ROADMAP_PROMPT.json

VALIDATION:
SELECT COUNT(*) FROM eap_prompt WHERE agent_code IN ('CST', 'CHG') -- Should be 7
SELECT COUNT(*) FROM eap_prompt WHERE prompt_code LIKE 'ANL_%' AND prompt_code NOT IN (existing) -- Should be 6
SELECT COUNT(*) FROM eap_prompt WHERE prompt_code LIKE 'DOC_%' AND prompt_code NOT IN (existing) -- Should be 2
```
...
### Prompt 9.1-9.4: Create ANL Financial Functions
```
TASK: Create Azure Functions for Complex Financial Calculations

LOCATION: environments/personal/functions/

These functions provide enhanced computation for the Personal environment.
They are NOT available in Mastercard (DLP blocked).
AI Builder prompts serve as fallback in Mastercard.
...
DEPLOYMENT:
1. Create function app in Azure (personal subscription)
2. Deploy all 4 functions
3. Configure CORS for Copilot Studio
4. Register endpoints in eap_capability_implementation

VALIDATION:
- Each function responds to test payload
- Response time < 5 seconds
- Error handling returns structured errors
```

### Prompt 9.5-9.6: Deploy and Register Functions
```
...
REGISTER IN eap_capability_implementation:

For Personal environment ONLY, add AZURE_FUNCTION implementations:
...
### Prompt 10.1: Create CST Test Scenarios
```
...
### Prompt 10.2: Create CHG Test Scenarios
```
...
### Prompt 10.3-10.5: Run Full Test Suite
```
...
### Prompt 11.1-11.2: Deploy to Personal Environment
```
...
### Prompt 11.3-11.4: Deploy to Mastercard Environment
```
...
### Prompt 11.5: Documentation Finalization
```
