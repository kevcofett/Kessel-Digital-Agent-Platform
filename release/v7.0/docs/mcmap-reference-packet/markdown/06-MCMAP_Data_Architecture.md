...
```
+-----------------------------------------------------------------------------+
|                      MCMAP DATA ARCHITECTURE                                  |
+-----------------------------------------------------------------------------+
|                                                                              |
|  +------------------------------------------------------------------------+ |
|  |                        DATAVERSE (Primary Store)                       | |
|  |                                                                        | |
|  |  +-----------------------------+  +-----------------------------+     | |
|  |  |     EAP PLATFORM TABLES     |  |     MPA DOMAIN TABLES       |     | |
|  |  |  +-----------------------+  |  |  +-----------------------+  |     | |
...
|                                     |                                        |
|  +---------------------------------------------------------------------+    |
|  |                    SHAREPOINT (Knowledge Store)                      |    |
|  |                                                                      |    |
|  |  +----------------------------------------------------------------+ |    |
|  |  |                    KNOWLEDGE BASE LIBRARY                      | |    |
|  |  |  - Agent Instructions (10 files)                               | |    |
|  |  |  - Core KB Files (10 files)                                    | |    |
|  |  |  - Deep Module KB Files (17+ files)                            | |    |
|  |  |  - EAP Shared KB Files (6 files)                               | |    |
|  |  +----------------------------------------------------------------+ |    |
|  |                                                                      |    |
|  +----------------------------------------------------------------------+    |
|                                                                              |
...
|                                                                              |
|  +------------------+         +------------------+                          |
|  |   eap_agent      |<--------|  eap_capability  |                          |
|  |                  |   1:N   |                  |                          |
|  |  - agent_code PK |         |  - capability_id |                          |
|  |  - agent_name    |         |  - agent_code FK |                          |
|  +------------------+         +--------+---------+                          |
|                                        |                                     |
|                                        | 1:N                                 |
|                                        v                                     |
...
|                                                                              |
|  +------------------+         +------------------+                          |
|  |   eap_prompt     |         |  eap_test_case   |                          |
|  |                  |         |                  |                          |
|  |  - prompt_code   |         |  - test_id       |                          |
|  |  - agent_code FK |         |  - capability FK |                          |
|  +------------------+         +------------------+                          |
|                                                                              |
|  +------------------+         +------------------+                          |
|  |   eap_session    |<--------|  eap_telemetry   |                          |
|  |                  |   1:N   |                  |                          |
|  |  - session_id PK |         |  - telemetry_id  |                          |
...
|  |                  |   |                  |   |                  |        |
|  |  - vertical_id   |   |  - channel_id    |   |  - kpi_id        |        |
|  |  - vertical_code |   |  - channel_code  |   |  - kpi_code      |        |
|  +--------+---------+   +--------+---------+   +--------+---------+        |
|           |                      |                      |                   |
|           +----------------------+----------------------+                   |
|                                  |                                          |
...
|                                                                              |
|  +------------------+         +------------------+                          |
|  |   mpa_partner    |         |   mpa_session    |                          |
|  |                  |         |                  |                          |
|  |  - partner_id    |         |  - session_id    |                          |
|  |  - partner_name  |         |  - step_data     |                          |
|  +------------------+         +------------------+                          |
|                                                                              |
+-----------------------------------------------------------------------------+
```
...
**Seed Data Count:** ~50 records (capabilities x environments)
...
**Purpose:** Industry benchmarks by vertical x channel x KPI
...
|  +--------+--------+                                                        |
|           |                                                                  |
|           | 1. Read agent routing                                           |
|           v                                                                  |
|  +-----------------------------------------------------------------+       |
|  |                         DATAVERSE                                |       |
|  |                                                                  |       |
|  |  +-------------+  +---------------------+  +-----------------+ |       |
|  |  |  eap_agent  |  |eap_capability_impl  |  |  mpa_benchmark  | |       |
|  |  |    READ     |  |       READ          |  |      READ       | |       |
|  |  +-------------+  +---------------------+  +-----------------+ |       |
|  |                                                                  |       |
|  |  +-------------+  +---------------------+                       |       |
...
|-------------|-------------------|---------|
| Schema Change | Architecture Review | RFC -> Review -> Implement -> Test -> Deploy |
| Reference Data Update | Domain Expert | Request -> Validate -> Import -> Verify |
| Configuration Change | Platform Admin | Request -> Test -> Deploy -> Verify |
| KB Content Update | Content Manager | Draft -> Review -> Publish |
